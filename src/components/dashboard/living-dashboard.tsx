...
import {
  ...
  Sparkles, Target, Utensils, WandSparkles, X, Scissors, ConciergeBell,
  type LucideIcon,
} from 'lucide-react';
...
const NAV: NavItem[] = [
  ...
  {label:'Beauty',href:'/beauty',icon:WandSparkles},{label:'Hair',href:'/hair',icon:Scissors},
  {label:'Brain',href:'/brain',icon:BrainCircuit},{label:'Concierge',href:'/concierge',icon:ConciergeBell},
...
function moodLabel(value: string | number | null | undefined) {
  ...
  return {title:label.charAt(0).toUpperCase()+label.slice(1),sub:'Today'};
}
function energyState(value:number|string|null|undefined){
  if(value==null)return {label:'—',sub:'Not logged',percent:null};
  if(typeof value==='number'){
    const percent=value<=5?Math.round((value/5)*100):value<=10?Math.round((value/10)*100):Math.min(100,Math.round(value));
    return {label:`${percent}%`,sub:percent>=75?'Good':percent>=50?'Steady':'Low',percent};
  }
  const labels:Record<string,string>={high:'High',medium:'Medium',low:'Low',exhausted:'Exhausted'};
  return {label:labels[value]??String(value),sub:'Energy',percent:null};
}
...
export function LivingDashboard({data,error,insight,userName,userImage}:{data:LivingDashboardData;error?:string;insight?:string|null;userName?:string;userImage?:string|null}){
  ...
  const habits=data.habitSummary.habits.slice(0,5);const energy=energyState(data.wellnessToday.entry?.energy);const mood=moodLabel(data.wellnessToday.entry?.mood);const water=Math.max(0,Math.round(data.wellnessToday.entry?.waterGlasses??0));const waterGoal=10;
  const top=tasks[0]??null;const routineCount=data.todayOverview.activeRoutines;const habitDone=data.habitSummary.completedToday;
  ...
  const taskCardTitle=data.todayOverview.tasksTotalToday>0?"Today's Tasks":'Top Tasks';
  ...
  return <div className="gd-world">
    ...
    <main className="gd-main">
      <header className="gd-hero">
        ...
        <div className="gd-greeting"><small>{data.greeting.label},</small><h1>{name}.</h1><p>{data.greeting.message||"You're right on track. Let's make today beautiful."}</p><div><Link href="/planning" className="primary">Plan My Day</Link><Link href="/calendar?view=day">View Full Day</Link></div></div>
      </header>
      ...
      <div className="gd-layout">
        <div className="gd-center">
          <div className="gd-kpis">
            ...
            <Glass className="gd-priority"><span className="gd-mini">✧ Top Priority</span><strong>{top?.title||'No priority selected'}</strong><span className="gd-priority-meta">{top?`${top.priority} priority${top.dueDate?` · ${fmtTime(new Date(top.dueDate))}`:''}`:'Open Tasks to choose one'}</span></Glass>
            <Glass className="gd-energy"><span className="gd-mini">✧ Energy</span><div className={`gd-ring ${energy.percent==null?'qualitative':''}`} style={energy.percent==null?undefined:{'--value':`${energy.percent}%`} as React.CSSProperties}><strong>{energy.label}</strong><small>{energy.sub}</small></div></Glass>
        ...
        <aside className="gd-right">
          <Glass><div className="gd-card-head"><h2>✧ Today at a Glance</h2></div><div className="gd-glance"><p><ListChecks size={15}/><span>Tasks</span><strong>{data.todayOverview.tasksTotalToday?`${data.todayOverview.tasksCompletedToday} / ${data.todayOverview.tasksTotalToday}`:'0 due'}</strong></p><p><CalendarDays size={15}/><span>Events</span><strong>{data.todayOverview.eventsToday}</strong></p><p><Sparkles size={15}/><span>Routines</span><strong>{routineCount}</strong></p><p><CheckCircle2 size={15}/><span>Habits</span><strong>{habitDone} / {data.habitSummary.totalHabits}</strong></p></div></Glass>
          <Glass><div className="gd-card-head"><h2>✧ Habit Progress</h2><Link href="/habits">View All</Link></div><div className="gd-habits">{habits.length?habits.map((habit,index)=>{const target=Math.max(1,habit.targetCount);const pct=Math.min(100,Math.round((habit.currentCount/target)*100));return <button key={habit.id} disabled={habit.completedToday||busyHabit===habit.id} onClick={()=>logHabit(habit.id)}><i style={{backgroundImage:`url(${index%2?WATER:BLOSSOM})`}}/><span><strong>{habit.name}</strong><em><b style={{width:`${pct}%`}}/></em></span><small>{habit.currentCount}/{target}</small></button>}):<div className="gd-empty">No habits configured.</div>}</div></Glass>
