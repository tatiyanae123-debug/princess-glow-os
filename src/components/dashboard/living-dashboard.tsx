    ...
    <div className="grid gap-[10px] px-[9px] pb-[18px] pt-0 xl:grid-cols-[minmax(0,964px)_304px]">
      <main className="min-w-0 space-y-[10px]">
        <Card className="grid min-h-[154px] grid-cols-2 rounded-t-none p-0 md:grid-cols-4">
          <Metric icon={Target} label="Today's Focus" title={topTask?.title ?? 'No priority task selected'} href={topTask?`/tasks?taskId=${encodeURIComponent(topTask.id)}&view=all`:'/tasks'} meta={<span>{topTask ? topTask.priority : 'Choose a focus'}</span>}/>
          ...
          <Metric icon={Sun} tone="gold" label="Current Routine" title={routine?.name ?? 'No routine active now'} href={routine?`/routines?routineId=${encodeURIComponent(routine.id)}`:'/routines'} meta={<span>{routine ? routine.timeOfDay : 'Open routines'}</span>}/>
        ...
        <div className="grid gap-[10px] lg:grid-cols-[280px_280px_minmax(0,1fr)]">
          <Card className="p-[15px]"><div className="flex items-center justify-between"><h2 className="text-[11px] font-medium">Today at a Glance</h2><Link href="/calendar" className="text-[8px] text-[#b85d72]">View all</Link></div><div className="mt-[10px] space-y-[3px]">{scheduled.slice(0,5).map((event)=><Link key={event.id} href={`/calendar?eventId=${encodeURIComponent(event.id)}&view=day`} className="grid min-h-[28px] grid-cols-[64px_1fr_auto] items-center gap-[6px] rounded-[5px] px-[5px] text-[8.5px] transition hover:bg-[#fae6e7]"><span>{event.allDay?'All day':fmtTime(event.startAt)}</span><span className="truncate">{event.title}</span><span className="text-[7px] text-[#9a918b]">{fmtDuration(event.startAt,event.endAt)}</span></Link>)}{scheduled.length===0?<p className="py-12 text-center text-[9px] text-[#918782]">Nothing scheduled today.</p>:null}</div></Card>
        ...
        <Card className="grid gap-3 p-[14px] sm:grid-cols-3">
          <Link href="/day-mode" className="rounded-[10px] bg-[#fae6e7] p-3"><Sparkles size={13} className="text-[#b85d72]"/><p className="mt-2 text-[9px] font-medium">Day Mode</p><p className="mt-1 text-[8px] text-[#8d8580]">Choose Productive, Bare Minimum, Clear Everything, or Highly Productive.</p></Link>
          <Link href="/knowledge" className="rounded-[10px] bg-[#f8f4f2] p-3"><NotebookPen size={13}/><p className="mt-2 text-[9px] font-medium">Notes + Memory</p><p className="mt-1 text-[8px] text-[#8d8580]">Capture a thought or open your knowledge system.</p></Link>
