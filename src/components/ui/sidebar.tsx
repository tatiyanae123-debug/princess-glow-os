...
export function Sidebar() {
  ...
  const editorial = ['/dashboard', '/tasks', '/calendar', '/planning', '/routines', '/habits', '/fitness'].includes(pathname);
  ...
  if (editorial) return (
    <aside className="editorial-sidebar">
      ...
      <nav>{navItems.slice(0, 12).map(item => { const Icon = item.icon; const active = pathname === item.href; return <Link key={item.href} href={item.href} className={active ? 'active' : ''}><Icon />{item.label === 'Tasks' ? 'Tasks & Planner' : item.label}</Link>; })}</nav>
      <Link className="all-rooms" href="/settings">View all rooms &nbsp; ⊙</Link>
    </aside>
  );
