...
export function Sidebar(){
  ...
  if(isDashboard){
    return <aside className="reference-sidebar" data-navigation-form="world-thresholds">
      <div>
        <Link href="/dashboard" className="reference-brand"><Sparkles/><span>GLOW OS</span></Link>
        <nav aria-label="Five Glow worlds">{PRIMARY.map(item=>{const navItem=byHref.get(item.href);const Icon=navItem?.icon??Sparkles;const active=navigationPathIsActive(pathname,item.href);return <Link key={item.href} href={item.href} className={active?'active':''} aria-current={active?'page':undefined}><Icon/><span>{item.label}</span></Link>;})}<hr/><Link href="/planning?allRooms=1"><CircleEllipsis/><span>Rooms</span></Link></nav>
      </div>
  ...
  return <aside className="glow-world-rail flex h-full w-full flex-col border-b border-[#ECECEC] bg-white px-3 py-3 lg:min-h-screen lg:border-b-0 lg:border-r lg:px-3 lg:py-5" data-navigation-form="world-thresholds">
    ...
    <div id="glow-navigation-panel" className={cn('mt-4 min-h-0 flex-1 border-t border-[#F3F3F3] pt-3 sm:mt-5 lg:mt-6 lg:border-t-0 lg:pt-0',mobileOpen?'block':'hidden lg:block')}>
      <nav aria-label="Five Glow worlds" className="space-y-1">
        {PRIMARY.map(item=>{
          ...
          return <Link key={item.label} href={item.href} onClick={closeMobile} aria-current={active?'page':undefined} className={cn('glow-world-threshold flex min-h-11 items-center gap-2.5 rounded-[8px] px-2.5 text-[12px] font-medium transition sm:min-h-10',active?'is-current bg-[#F8EFF1] text-[#9A5363]':'text-[#343438] hover:bg-[#F8F8F8]')} data-world-label={item.label.toLowerCase()}>{Icon?<Icon size={14} strokeWidth={1.6}/>:<span className="h-3.5 w-3.5"/>}<span>{item.label}</span></Link>;
        })}
      </nav>
