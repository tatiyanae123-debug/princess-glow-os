...
const SESSION_KEY = 'glow.gua-sha.session.v4';
const VIEW_PATHS: Record<GuaShaStudioView, string> = {
  today: '/beauty/gua-sha',
  guided: '/beauty/gua-sha/guided',
  morning: '/beauty/gua-sha/morning',
  midday: '/beauty/gua-sha/midday',
  night: '/beauty/gua-sha/night',
};
...
function movementPaths(step: GuaShaMovementStep) {
  const name = `${step.id} ${step.name}`.toLowerCase();
  if (/under-eye|eye|temple/.test(name)) return ['M46 40 C40 40 35 38 31 35', 'M54 40 C60 40 65 38 69 35', 'M44 43 C38 45 34 43 31 40', 'M56 43 C62 45 66 43 69 40'];
  if (/brow|forehead/.test(name)) return ['M44 28 C43 20 44 13 45 8', 'M50 28 V7', 'M56 28 C57 20 56 13 55 8', 'M41 29 C38 25 37 21 38 17', 'M59 29 C62 25 63 21 62 17'];
  if (/neck|collarbone/.test(name)) return ['M45 69 C44 79 44 89 44 97', 'M50 69 V98', 'M55 69 C56 79 56 89 56 97'];
  if (/jaw|chin/.test(name)) return ['M46 62 C38 61 31 57 25 51', 'M54 62 C62 61 69 57 75 51', 'M47 66 C39 66 33 62 28 57', 'M53 66 C61 66 67 62 72 57'];
  if (/slip|prepare/.test(name)) return [];
  return ['M48 46 C42 46 36 42 31 37', 'M52 46 C58 46 64 42 69 37', 'M47 51 C40 51 34 47 29 42', 'M53 51 C60 51 66 47 71 42'];
}

function MovementOverlay({ step, view, mirror, visible }: { step: GuaShaMovementStep | null; view: SessionView; mirror: boolean; visible: boolean }) {
  ...
  const active = movementPaths(step);
  const showContext = view === 'guided' || view === 'night';
  return <svg className={styles.routeSvg} viewBox="0 0 100 100" preserveAspectRatio="none" style={{ transform: mirror ? 'scaleX(-1)' : undefined }} aria-hidden="true"><defs><marker id={`arrow-${view}`} markerWidth="4" markerHeight="4" refX="3.1" refY="1.7" orient="auto"><path d="M0,0 L0,3.4 L3.5,1.7 z" fill="rgba(255,255,255,.95)" /></marker></defs>{showContext ? contextPaths.map((d, index) => <path key={`context-${index}`} d={d} className={`${styles.route} ${index % 2 ? styles.routeDashed : ''}`} markerEnd={`url(#arrow-${view})`} />) : null}{active.map((d, index) => <path key={`active-${index}`} d={d} className={`${styles.route} ${index % 2 ? styles.routeDashed : ''} ${view === 'night' && index === 0 ? styles.routeActive : ''}`} markerEnd={`url(#arrow-${view})`} />)}</svg>;
}
...
function RoutineChooser({ onClose, onSelect }: { onClose: () => void; onSelect: (view: SessionView) => void }) {
  const cards: { view: SessionView; title: string; subtitle: string; icon: LucideIcon }[] = [
  ...
  return <div className={styles.routineOverlay} role="dialog" aria-modal="true" aria-label="Gua Sha routines"><div className={styles.routineSheet}><header><div><h2>Gua Sha Routines</h2><p>Choose the ritual that fits your time and energy. Your current Gua Sha session stays with you.</p></div><button type="button" onClick={onClose} aria-label="Close routines"><X size={18} /></button></header><div className={styles.routineCards}>{cards.map(({ view, title, subtitle, icon: Icon }) => <button key={view} type="button" className={styles.routineCard} onClick={() => onSelect(view)}><span><Icon size={21} /></span><span><strong>{title}</strong><small>{subtitle}</small></span><ChevronRight size={17} /></button>)}</div></div></div>;
}

export function GuaShaReferenceStudioV3({ view, savedRoutineSteps, ownedTools, linkedSlipProducts, userName, userImage }: Props) {
  ...
  const [playing, setPlaying] = useState(view === 'midday');
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        ...
        setNightCompatible(saved.nightCompatible ?? false);
  ...
  useEffect(() => {
    if (!hydrated) return;
    let previous: PersistedSession | null = null;
    try {
      const raw = sessionStorage.getItem(SESSION_KEY);
      previous = raw ? JSON.parse(raw) as PersistedSession : null;
    } catch {}
    const next: PersistedSession = {
      ...
      breathCue,
      nightCompatible,
      durationByView: { ...(previous?.durationByView ?? {}), ...(view === 'today' ? {} : { [view]: duration }) },
      repsByView: { ...(previous?.repsByView ?? {}), ...(view === 'today' ? {} : { [view]: reps }) },
      stepByView: { ...(previous?.stepByView ?? {}), ...(view === 'today' ? {} : { [view]: safeStepIndex }) },
      slipAppliedAt,
    };
    try { sessionStorage.setItem(SESSION_KEY, JSON.stringify(next)); } catch {}
  }, [hydrated, view, careMode, concerns, readiness, readinessTouched, followMode, toolMode, mirror, voice, voiceSpeed, breathCue, nightCompatible, duration, reps, safeStepIndex, slipAppliedAt]);

  useEffect(() => {
    if (!step) return;
    setRemaining(effectiveSeconds);
    setPlaying(view === 'midday');
    setEnded(false);
  }, [step?.id, effectiveSeconds, view]);
  ...
  }, [playing, voice, voiceSpeed, followMode, step]);
  ...
  useEffect(() => {
    document.dispatchEvent(new CustomEvent('glow:context', { detail: {
      ...
      readinessTouched,
      concerns,
      session: step ? { stepId: step.id, stepName: step.name, zone: step.zone, pressure: step.pressure, duration, reps, followMode, toolMode, mirror, voice, slipProduct } : null,
    }}));
  }, [view, careMode, readiness, readinessTouched, concerns, step, duration, reps, followMode, toolMode, mirror, voice, slipProduct]);
  ...
  function setRiskSafe(key: 'irritated' | 'broken', safe: boolean) {
    setReadiness((current) => ({ ...current, [key]: !safe }));
    setReadinessTouched((current) => current.includes(key) ? current : [...current, key]);
  }
  ...
  function go(nextView: SessionView) {
    travel(VIEW_PATHS[nextView]);
  }

  async function expandStage() {
    try { if (stageRef.current?.requestFullscreen) await stageRef.current.requestFullscreen(); } catch {}
  }
  ...
  return <main className={styles.pageRoot}><section className={`${styles.studio} ${view === 'night' ? styles.night : ''}`} data-room-local-identity="gua-sha-studio" data-gua-sha-view={view}>
    ...
    <section className={styles.workArea}>
      ...
      {view === 'today' ? <TodayExperience readiness={readiness} readinessTouched={readinessTouched} toggleReadiness={toggleReadiness} concerns={concerns} toggleConcern={toggleConcern} careMode={careMode} setCareMode={setCareMode} ready={ready} unsafe={unsafe} onStart={go} stageRef={stageRef} onExpand={expandStage} /> : <SessionExperience view={view} steps={steps} step={step} stepIndex={safeStepIndex} setStepIndex={setStepIndex} duration={duration} setDuration={setDuration} reps={reps} setReps={setReps} followMode={followMode} setFollowMode={setFollowMode} toolMode={toolMode} setToolMode={setToolMode} mirror={mirror} setMirror={setMirror} voice={voice} setVoice={setVoice} voiceSpeed={voiceSpeed} setVoiceSpeed={setVoiceSpeed} breathCue={breathCue} setBreathCue={setBreathCue} playing={playing} setPlaying={setPlaying} remaining={remaining} effectiveSeconds={effectiveSeconds} slipProduct={slipProduct} slipAppliedAt={slipAppliedAt} setSlipAppliedAt={setSlipAppliedAt} readiness={readiness} readinessTouched={readinessTouched} setRiskSafe={setRiskSafe} nightCompatible={nightCompatible} setNightCompatible={setNightCompatible} endSession={endSession} stageRef={stageRef} onExpand={expandStage} />}
    ...
    {routineChooserOpen ? <RoutineChooser onClose={() => setRoutineChooserOpen(false)} onSelect={(next) => { setRoutineChooserOpen(false); go(next); }} /> : null}
  </section></main>;
}
...
function TodayExperience({ readiness, readinessTouched, toggleReadiness, concerns, toggleConcern, careMode, setCareMode, ready, unsafe, onStart, stageRef, onExpand }: {
  ...
  ready: boolean;
  unsafe: boolean;
  onStart: (view: SessionView) => void;
  stageRef: RefLike;
  onExpand: () => void;
}) {
  const rows: { key: ReadinessKey; title: string; subtitle: string; icon: LucideIcon }[] = [
    ...
    { key: 'irritated', title: 'Skin irritated', subtitle: 'Red, inflamed or reactive', icon: Sparkles },
    { key: 'broken', title: 'Broken skin', subtitle: 'Cuts, acne or open areas', icon: Bandage },
    { key: 'procedure', title: 'Recent procedure', subtitle: 'Facial, injectables, peel, etc.', icon: Plus },
    { key: 'time', title: 'Time available', subtitle: 'Do you have a few quiet minutes?', icon: Clock3 },
  ];
  const concernRows: { label: string; icon: LucideIcon }[] = [
    { label: 'Puffiness', icon: Sparkles }, { label: 'Feeling fine', icon: Circle }, { label: 'Jaw tension', icon: Heart }, { label: 'Skin sensitive today', icon: Leaf }, { label: 'Temple tension', icon: Circle }, { label: 'Recently used strong skincare', icon: Droplet }, { label: 'Scalp tension', icon: Circle }, { label: 'Procedure recovery', icon: Plus }, { label: 'Neck tightness', icon: Expand }, { label: 'Just want relaxation', icon: Leaf },
  ];
  const modes: { id: CareMode; title: string; subtitle: string; icon: LucideIcon }[] = [
  ...
  let recommendedView: SessionView = 'guided';
...
function SessionExperience({ view, steps, step, stepIndex, setStepIndex, duration, setDuration, reps, setReps, followMode, setFollowMode, toolMode, setToolMode, mirror, setMirror, voice, setVoice, voiceSpeed, setVoiceSpeed, breathCue, setBreathCue, playing, setPlaying, remaining, effectiveSeconds, slipProduct, slipAppliedAt, setSlipAppliedAt, readiness, readinessTouched, setRiskSafe, nightCompatible, setNightCompatible, endSession, stageRef, onExpand }: {
  ...
  readiness: Record<ReadinessKey, boolean>;
  readinessTouched: ReadinessKey[];
  setRiskSafe: (key: 'irritated' | 'broken', safe: boolean) => void;
  nightCompatible: boolean;
  setNightCompatible: (value: boolean) => void;
  endSession: () => void;
  stageRef: RefLike;
  onExpand: () => void;
}) {
  ...
  const closedEyes = view === 'midday' || (view === 'guided' && (zone === 'Eyes' || zone === 'Forehead'));
  const portrait = closedEyes ? CLOSED_PORTRAIT : OPEN_PORTRAIT;
  const previous = () => setStepIndex(Math.max(0, stepIndex - 1));
  const next = () => setStepIndex(Math.min(Math.max(0, steps.length - 1), stepIndex + 1));
  const totalRemaining = Math.max(remaining, remaining + steps.slice(stepIndex + 1).reduce((sum, item) => sum + Math.round(item.seconds * ((duration * 60) / Math.max(1, steps.reduce((acc, s) => acc + s.seconds, 0)))), 0));

  return <div className={styles.workspace}>
    <aside className={styles.leftCol}>{view === 'guided' ? <GuidedLeft toolMode={toolMode} setToolMode={setToolMode} followMode={followMode} setFollowMode={setFollowMode} duration={duration} setDuration={setDuration} mirror={mirror} setMirror={setMirror} voice={voice} setVoice={setVoice} voiceSpeed={voiceSpeed} setVoiceSpeed={setVoiceSpeed} reps={reps} setReps={setReps} /> : null}{view === 'morning' ? <MorningLeft toolMode={toolMode} setToolMode={setToolMode} followMode={followMode} setFollowMode={setFollowMode} duration={duration} setDuration={setDuration} mirror={mirror} setMirror={setMirror} voice={voice} setVoice={setVoice} reps={reps} setReps={setReps} /> : null}{view === 'midday' ? <MiddayLeft followMode={followMode} setFollowMode={setFollowMode} toolMode={toolMode} setToolMode={setToolMode} steps={steps} stepIndex={stepIndex} setStepIndex={setStepIndex} /> : null}{view === 'night' ? <NightLeft steps={steps} stepIndex={stepIndex} setStepIndex={setStepIndex} duration={duration} setDuration={setDuration} pressure={pressure} voice={voice} setVoice={setVoice} mirror={mirror} setMirror={setMirror} breathCue={breathCue} setBreathCue={setBreathCue} /> : null}</aside>

    <section className={styles.stage} ref={stageRef}><img className={`${styles.portrait} ${closedEyes ? styles.portraitClosed : ''}`} src={portrait} alt="Photographic face used as the Gua Sha movement canvas" /><div className={styles.portraitVeil} /><div className={styles.stepBadge}><span>Step</span><strong>{step ? `${displayedStep} of ${steps.length}` : '— of —'}</strong></div><button type="button" className={styles.expand} onClick={onExpand} aria-label="Expand facial guide"><Maximize2 size={17} /></button><MovementOverlay step={step} view={view} mirror={mirror} visible={followMode === 'guided'} />
      ...
      {view === 'morning' ? <><div className={styles.morningFacts}><div className={styles.factTop}><span><small>Step {stepIndex + 1} of {steps.length}</small><strong>{step?.name ?? 'Cheek sweep'}</strong><small>{step?.subtitle ?? 'Upward and outward'}</small></span><span className={styles.liveClock}><Clock3 size={17} />{formatClock(totalRemaining)}</span></div><div className={styles.factGrid}><span><b>Start point</b>{step?.startPoint ?? 'Side of nose'}</span><span><b>End point</b>{step?.endPoint ?? 'Cheekbone toward ear'}</span><span><b>Pressure</b>{pressure}</span><span><b>Repetitions</b>{reps} each side</span><span><b>Tool edge</b>{step?.toolEdge ?? 'Broad curved edge'}</span></div></div><Transport previous={previous} next={next} playing={playing} setPlaying={setPlaying} first={stepIndex === 0} last={stepIndex === steps.length - 1} /></> : null}
    ...
    <aside className={styles.rightCol}>{view === 'guided' ? <GuidedRight zone={zone} pressure={pressure} slipProduct={slipProduct} slipAppliedAt={slipAppliedAt} setSlipAppliedAt={setSlipAppliedAt} /> : null}{view === 'morning' ? <MorningRight steps={steps} stepIndex={stepIndex} setStepIndex={setStepIndex} zone={zone} slipProduct={slipProduct} /> : null}{view === 'midday' ? <MiddayRight slipProduct={slipProduct} /> : null}{view === 'night' ? <NightRight slipProduct={slipProduct} setSlipAppliedAt={setSlipAppliedAt} readiness={readiness} readinessTouched={readinessTouched} setRiskSafe={setRiskSafe} nightCompatible={nightCompatible} setNightCompatible={setNightCompatible} /> : null}</aside>
  </div>;
}

function GuidedLeft({ toolMode, setToolMode, followMode, setFollowMode, duration, setDuration, mirror, setMirror, voice, setVoice, voiceSpeed, setVoiceSpeed, reps, setReps }: { toolMode: ToolMode; setToolMode: (value: ToolMode) => void; followMode: FollowMode; setFollowMode: (value: FollowMode) => void; duration: number; setDuration: (value: number) => void; mirror: boolean; setMirror: (value: boolean) => void; voice: boolean; setVoice: (value: boolean) => void; voiceSpeed: number; setVoiceSpeed: (value: number) => void; reps: number; setReps: (value: number) => void }) {
  return <><ModePanel toolMode={toolMode} setToolMode={setToolMode} followMode={followMode} setFollowMode={setFollowMode} /><DurationPanel duration={duration} setDuration={setDuration} /><section className={styles.panel}><h2>Follow along</h2><div className={styles.segmented}><button type="button" className={followMode === 'guided' ? styles.active : ''} onClick={() => setFollowMode('guided')}>Guided</button><button type="button" className={followMode === 'freestyle' ? styles.active : ''} onClick={() => setFollowMode('freestyle')}>Freestyle</button></div><div className={styles.settingRow}><Volume2 size={18} /><span>Voice over</span><Toggle checked={voice} onChange={() => setVoice(!voice)} label="Voice over" /></div><div className={styles.settingRow}><span /><span>Voice speed</span><strong>{voiceSpeed.toFixed(1)}x</strong></div><input type="range" min=".75" max="1.5" step=".25" value={voiceSpeed} onChange={(event) => setVoiceSpeed(Number(event.target.value))} /></section><section className={styles.panel}><div className={styles.settingRow}><Repeat2 size={18} /><span>Mirror mode</span><Toggle checked={mirror} onChange={() => setMirror(!mirror)} label="Mirror mode" /></div><div className={styles.mirrorPreview}><MiniFace zone="Cheeks" /><span>↔</span><MiniFace zone="Cheeks" /></div></section><section className={styles.panel}><h2>Repetitions</h2><RepControl reps={reps} setReps={setReps} /></section></>;
}

function MorningLeft({ toolMode, setToolMode, followMode, setFollowMode, duration, setDuration, mirror, setMirror, voice, setVoice, reps, setReps }: { toolMode: ToolMode; setToolMode: (value: ToolMode) => void; followMode: FollowMode; setFollowMode: (value: FollowMode) => void; duration: number; setDuration: (value: number) => void; mirror: boolean; setMirror: (value: boolean) => void; voice: boolean; setVoice: (value: boolean) => void; reps: number; setReps: (value: number) => void }) {
  return <><DurationPanel duration={duration} setDuration={setDuration} morning /><ModePanel toolMode={toolMode} setToolMode={setToolMode} followMode={followMode} setFollowMode={setFollowMode} morning /><section className={styles.panel}><div className={styles.settingRow}><Volume2 size={18} /><span>Voice guidance<br /><small>Gentle & clear</small></span><Toggle checked={voice} onChange={() => setVoice(!voice)} label="Voice guidance" /></div></section><section className={styles.panel}><div className={styles.settingRow}><Repeat2 size={18} /><span>Mirror mode<br /><small>Mirror directions left ↔ right</small></span><Toggle checked={mirror} onChange={() => setMirror(!mirror)} label="Mirror mode" /></div><div className={styles.mirrorPreview}><MiniFace zone="Cheeks" /><span>↔</span><MiniFace zone="Cheeks" /></div></section><section className={styles.panel}><h2>Repetitions <small>(per side)</small></h2><RepControl reps={reps} setReps={setReps} /></section><section className={styles.panel}><Sun size={24} /><h2>A brighter you</h2><p>A quick daily ritual to reduce puffiness, release tension, and wake up your natural glow.</p></section></>;
}

function MiddayLeft({ followMode, setFollowMode, toolMode, setToolMode, steps, stepIndex, setStepIndex }: { followMode: FollowMode; setFollowMode: (value: FollowMode) => void; toolMode: ToolMode; setToolMode: (value: ToolMode) => void; steps: GuaShaMovementStep[]; stepIndex: number; setStepIndex: (value: number) => void }) {
  return <><section className={styles.panel}><h2>Quick settings</h2><div className={styles.durationValue}><Clock3 size={24} /><strong>2–3</strong><span>min ⌄</span></div><div className={styles.segmented}><button type="button" className={followMode === 'guided' ? styles.active : ''} onClick={() => setFollowMode('guided')}>Guided</button><button type="button" className={followMode === 'freestyle' ? styles.active : ''} onClick={() => setFollowMode('freestyle')}>Off</button></div><div className={styles.settingRow}><Hand size={18} /><span>Use hands instead<br /><small>Same movements, no tool needed.</small></span><Toggle checked={toolMode === 'hands'} onChange={() => setToolMode(toolMode === 'hands' ? 'tool' : 'hands')} label="Use hands instead" /></div></section><section className={`${styles.panel} ${styles.resetPath}`}><h2>The reset path</h2>{steps.map((item, index) => <button type="button" key={item.id} onClick={() => setStepIndex(index)}><span className={styles.thumb} /><i>{index + 1}</i><span><strong>{item.name}</strong><small>{item.subtitle}</small><b>{item.seconds} sec</b></span></button>)}</section></>;
}

function NightLeft({ steps, stepIndex, setStepIndex, duration, setDuration, pressure, voice, setVoice, mirror, setMirror, breathCue, setBreathCue }: { steps: GuaShaMovementStep[]; stepIndex: number; setStepIndex: (value: number) => void; duration: number; setDuration: (value: number) => void; pressure: GuaShaPressure; voice: boolean; setVoice: (value: boolean) => void; mirror: boolean; setMirror: (value: boolean) => void; breathCue: boolean; setBreathCue: (value: boolean) => void }) {
  return <><section className={`${styles.panel} ${styles.timeline}`}><h2>Evening session</h2><p>A longer, restorative routine</p>{steps.map((item, index) => <button type="button" key={item.id} className={index === stepIndex ? styles.active : ''} onClick={() => setStepIndex(index)}><i /><span><strong>{item.name}</strong><small>{item.subtitle}</small></span></button>)}</section><section className={styles.panel}><h2>Session settings</h2><div className={styles.settingRow}><Clock3 size={17} /><span>Duration</span><select aria-label="Night routine duration" value={duration} onChange={(event) => setDuration(Number(event.target.value))}><option value={10}>10 min</option><option value={15}>15 min</option><option value={20}>20 min</option></select></div><div className={styles.settingRow}><Leaf size={17} /><span>Pressure</span><strong>{pressure}</strong></div><div className={styles.settingRow}><Volume2 size={17} /><span>Voice guidance</span><Toggle checked={voice} onChange={() => setVoice(!voice)} label="Voice guidance" /></div><div className={styles.settingRow}><Repeat2 size={17} /><span>Mirror mode</span><Toggle checked={mirror} onChange={() => setMirror(!mirror)} label="Mirror mode" /></div><div className={styles.settingRow}><Expand size={17} /><span>Breath cue</span><Toggle checked={breathCue} onChange={() => setBreathCue(!breathCue)} label="Breath cue" /></div></section></>;
}

function RepControl({ reps, setReps }: { reps: number; setReps: (value: number) => void }) {
  return <div className={styles.repRow}><Repeat2 size={22} /><strong>{reps}</strong><span>reps</span><div><button type="button" onClick={() => setReps(Math.max(1, reps - 1))}>−</button><button type="button" onClick={() => setReps(Math.min(20, reps + 1))}>＋</button></div></div>;
}

function GuidedRight({ zone, pressure, slipProduct, slipAppliedAt, setSlipAppliedAt }: { zone: GuaShaZone; pressure: GuaShaPressure; slipProduct: string | null; slipAppliedAt: string | null; setSlipAppliedAt: (value: string | null) => void }) {
  return <><section className={styles.panel}><h2>Active zone</h2><MiniFace zone={zone} /></section><section className={styles.panel}><h2>Pressure guide</h2><div className={styles.rangeLabels}><span>Light</span><span>Medium</span><span>Firm</span></div><PressureDots pressure={pressure} /></section><section className={styles.panel}><h2>Tools & slip</h2><p>Recommended</p><ProductCard product={slipProduct} /><button type="button" className={styles.actionRow} onClick={() => setSlipAppliedAt(new Date().toISOString())}><Droplet size={16} /><span>Apply more slip</span><span>{slipAppliedAt ? '✓' : ''}</span></button><button type="button" className={styles.actionRow} onClick={() => travel('/beauty/skincare?view=product-library')}><Plus size={16} /><span>Add your own</span><ChevronRight size={14} /></button></section><section className={styles.panel}><h2>Today’s focus</h2><p>Release tension</p><strong>{zone === 'Jaw' ? 'Jaw · Cheeks · Eyes' : `${zone} · gentle movement`}</strong></section></>;
}

function MorningRight({ steps, stepIndex, setStepIndex, zone, slipProduct }: { steps: GuaShaMovementStep[]; stepIndex: number; setStepIndex: (value: number) => void; zone: GuaShaZone; slipProduct: string | null }) {
  return <><section className={`${styles.panel} ${styles.sequence}`}><h2>Your morning sequence</h2>{steps.map((item, index) => <button type="button" key={item.id} className={index === stepIndex ? styles.active : ''} onClick={() => setStepIndex(index)}><i>{index + 1}</i><span><strong>{item.name}</strong><small>{item.subtitle}</small></span><em>~ {Math.max(1, Math.round(item.seconds / 60))} min</em></button>)}</section><section className={styles.panel}><h2>Today’s morning fit</h2><p>Active zone: {zone}</p><div className={styles.recommendBody}><MiniFace zone={zone} /><ul className={styles.reasonList}><li><Check size={12} />Depuffs and brightens</li><li><Check size={12} />Relaxes facial tension</li><li><Check size={12} />Boosts circulation</li><li><Check size={12} />Wakes up your glow</li></ul></div></section><section className={styles.panel}><h2>Slip check</h2><ProductCard product={slipProduct} /><div className={styles.benefitRow}><Check size={18} /><span><strong>{slipProduct ? 'Good slip — you’re ready' : 'Slip not verified'}</strong><small>{slipProduct ? 'A linked slip product is available.' : 'Choose a known compatible product first.'}</small></span></div></section><section className={styles.panel}><Leaf size={24} /><strong>Best for:</strong><p>Puffiness, jaw tension, waking up the face.</p></section></>;
}

function MiddayRight({ slipProduct }: { slipProduct: string | null }) {
  return <><section className={styles.panel}><Sun size={29} /><h2>Desk break reset</h2><p>A quick release to feel lighter, clearer, and more you.</p></section><section className={styles.panel}><h2>Best when</h2><div className={styles.careModes}><span>Midday tension</span><span>Screen fatigue</span><span>Jaw tightness</span><span>Quick refresh</span></div></section><section className={styles.panel}><h2>Helpful cues</h2><div className={styles.cueRow}><MiniFace zone="Jaw" /><span><strong>Unclench your jaw</strong><small>Let your teeth slightly part.</small></span></div><div className={styles.cueRow}><Expand size={25} /><span><strong>Shoulders back</strong><small>Soften and drop.</small></span></div><div className={styles.cueRow}><Heart size={24} /><span><strong>Take four slow breaths</strong><small>In through the nose, out longer.</small></span></div></section><section className={styles.panel}><h2>Tools & slip</h2><ProductCard product={slipProduct} /></section></>;
}

function NightRight({ slipProduct, setSlipAppliedAt, readiness, readinessTouched, setRiskSafe, nightCompatible, setNightCompatible }: { slipProduct: string | null; setSlipAppliedAt: (value: string | null) => void; readiness: Record<ReadinessKey, boolean>; readinessTouched: ReadinessKey[]; setRiskSafe: (key: 'irritated' | 'broken', safe: boolean) => void; nightCompatible: boolean; setNightCompatible: (value: boolean) => void }) {
  const calmConfirmed = readinessTouched.includes('irritated') && !readiness.irritated;
  const noBrokenConfirmed = readinessTouched.includes('broken') && !readiness.broken;
  return <><section className={styles.panel}><h2>Tonight’s slip</h2><p>Nourish. Protect. Glide.</p><ProductCard product={slipProduct} /><div className={styles.benefitRow}><Leaf size={18} /><span><strong>Great for tonight</strong><small>Supports a slower evening ritual</small></span></div><div className={styles.benefitRow}><Droplet size={18} /><span><strong>Enough glide</strong><small>Keeps the tool from dragging</small></span></div><div className={styles.benefitRow}><Sparkles size={18} /><span><strong>Works with your skincare</strong><small>Confirm compatibility with active ingredients</small></span></div><button type="button" className={styles.actionRow} onClick={() => setSlipAppliedAt(new Date().toISOString())}><Droplet size={18} /><span>Reapply slip</span><Plus size={18} /></button></section><section className={styles.panel}><h2>Evening safety</h2><p>Check in before you begin</p><div className={styles.safetyRow}><Leaf size={18} /><span><strong>Skin feels calm</strong><small>Not irritated or reactive</small></span><Toggle checked={calmConfirmed} onChange={() => setRiskSafe('irritated', !calmConfirmed)} label="Skin feels calm" /></div><div className={styles.safetyRow}><Heart size={18} /><span><strong>No broken skin</strong><small>Cuts, acne or open areas</small></span><Toggle checked={noBrokenConfirmed} onChange={() => setRiskSafe('broken', !noBrokenConfirmed)} label="No broken skin" /></div><div className={styles.safetyRow}><Sparkles size={18} /><span><strong>Compatible with tonight’s skincare</strong><small>Confirm when active ingredients are involved</small></span><Toggle checked={nightCompatible} onChange={() => setNightCompatible(!nightCompatible)} label="Compatible with tonight skincare" /></div></section><section className={styles.panel}><Moon size={34} /><h2>A calmer you</h2><p>Release the day.<br />Sculpt with care.<br />Rest in your glow.</p></section></>;
}
