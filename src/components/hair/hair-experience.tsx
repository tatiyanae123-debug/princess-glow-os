...
const D = 86400000;
const F = 'w-full rounded-lg border border-[#EEE3DE] bg-white px-3 py-2 text-[11px]';
...
const masterRoutines: MasterRoutine[] = [
  {
    ...
    steps: [
      ...
      { title: 'Lightly rehydrate', detail: 'Mist mids and ends lightly with aloe vera juice + water, or plain water if already moisturized. Focus on dryness, frizz and ends. Do not soak roots.' },
      { title: 'Apply leave-in', detail: 'Use a small amount on mids, ends and dry areas. Avoid roots, scalp and the top of leave-out.' },
      { title: 'Seal moisture', detail: 'Use 2–3 drops maximum of jojoba oil, lightweight serum or hair oil on mids and ends. Avoid greasy buildup.' },
      { title: 'Check leave-out + front', detail: 'Separate the front section. Check puffiness, frizz, blending and parting. Use a spoolie, edge brush or small comb.' },
      { title: 'Style main hair', detail: 'Straighten, wand curl, roller-set, layer curls, brush out curls, sleek or add soft volume while keeping movement and softness.' },
      { title: 'Curl front pieces LAST', detail: 'Finish with bangs, face-framing pieces, leave-out or front layers. Create soft bends, movement and airy face framing, not stiff or crunchy curls.' },
    ],
  },
  {
    id: 'night',
    ...
    steps: [
      ...
      { title: 'Detangle gently', detail: 'Use fingers first, then a wide-tooth comb if needed. Start at the ends and work upward slowly.' },
      { title: 'Moisture seal', detail: 'Every night or every other night, lightly mist water or aloe mix, use a small amount of leave-in, then seal ends with 2–3 drops of light oil.' },
      { title: 'Scalp care', detail: 'Massage with fingertips for 3–5 minutes. Optional scalp serum or rosemary oil according to your routine.' },
      { title: 'Secure loosely', detail: 'Choose 2 loose braids, twists, a low loose bun, loose wrap, flexi rod, pineapple or silk wrap.' },
      { title: 'Protect while sleeping', detail: 'Use a silk/satin scarf or bonnet and, when available, a satin pillowcase.' },
    ],
  },
  {
    id: 'sunday',
    ...
    steps: [
      ...
      { title: 'Shampoo #1', detail: 'Cleanse buildup. Use Pantene Volume when oily, based on your saved routine.' },
      { title: 'Shampoo #2', detail: 'Target the result with L’Oréal gloss shampoo for shine or a repair shampoo.' },
      { title: 'Hair mask', detail: 'Apply mask from mid-lengths to ends for 5–10 minutes.' },
      { title: 'Condition + detangle', detail: 'Condition mids and ends and detangle gently from ends upward while there is slip.' },
      { title: 'Rinse gently', detail: 'Use warm-to-cool water and avoid extremely hot water.' },
      { title: 'Dry without friction', detail: 'Use a microfiber towel or cotton T-shirt. Squeeze and press, never aggressively rub.' },
      { title: 'Post-shower care', detail: 'Apply leave-in conditioner, heat protectant, then blow dry or style as planned.' },
    ],
  },
  {
    id: 'thursday',
    ...
    steps: [
      ...
      { title: 'Dry gently', detail: 'Use a microfiber towel or cotton T-shirt and squeeze or press out water.' },
      { title: 'Leave-in', detail: 'Apply leave-in conditioner through mids and ends.' },
      { title: 'Style or prep', detail: 'Restyle, refresh leave-out, or prep the hair for the next look.' },
    ],
  },
  {
    id: 'scalp-oil',
    ...
    steps: [
      ...
      { title: 'Treatment window', detail: 'Leave on for 1–4 hours or overnight according to the saved routine.' },
      { title: 'Wash out', detail: 'Cleanse afterward rather than layering oil on the scalp for days.' },
    ],
  },
];
...
const weeklyPlan = [
  ...
  ['Friday', 'Scalp massage + maintenance'],
...
export function HairExperience({ logs, timeline, routines, routineSteps, events, goals }: { logs: H[]; timeline: T[]; routines: Routine[]; routineSteps: RoutineStep[]; events: CalendarEvent[]; goals: Goal[] }) {
  ...
  const [tab, setTab] = useState<'today' | 'routines' | 'week' | 'products' | 'journey'>('today');
  const [completed, setCompleted] = useState<Record<string, boolean>>({});

  const w = logs.filter((x) => wash(x.eventType));
  const last = logs[0];
  const interval = w.length > 1 ? Math.max(1, Math.round((w[0].occurredAt.getTime() - w[1].occurredAt.getTime()) / D)) : 7;
  const next = new Date((w[0]?.occurredAt ?? new Date()).getTime() + interval * D);
  const heat = logs.filter((x) => x.heatUsed && Date.now() - x.occurredAt.getTime() < 30 * D).length;
  ...
  const appts = events.filter((e) => e.startAt.getTime() >= Date.now() && /hair|salon|trim|color|braid|stylist|blowout/i.test(`${e.title} ${e.description ?? ''}`)).sort((a, b) => a.startAt.getTime() - b.startAt.getTime()).slice(0, 4);
  ...
  const suggestedRoutine = useMemo(() => {
    ...
    return hour >= 18 ? masterRoutines[1] : masterRoutines[0];
  }, [todayRoutine]);
    ...
    <div className="mx-auto max-w-[1240px] space-y-5 pb-10">
      ...
      <nav className="flex gap-2 overflow-x-auto pb-1" aria-label="Hair workspace">
        {([
          ...
          <button key={key} onClick={() => setTab(key)} className={`whitespace-nowrap rounded-full border px-4 py-2 text-[10px] transition ${tab === key ? 'border-[#C9727E] bg-[#F8E6E7] text-[#864D58]' : 'border-[#EEE3DE] bg-white text-[#8B7D75] hover:bg-[#FCF8F6]'}`}>{label}</button>
        ))}
      </nav>
      ...
      {tab === 'today' ? <>
        <section className="grid gap-4 lg:grid-cols-[1.4fr_.8fr]">
          <div className="relative overflow-hidden rounded-[28px] border border-[#EEDDD7] bg-[radial-gradient(circle_at_82%_18%,rgba(255,255,255,.86),transparent_28%),linear-gradient(135deg,#EADAD2,#F8EFEB_55%,#EFE0DC)] p-6 shadow-[0_22px_70px_rgba(94,65,56,.08)] sm:p-8">
            <Waves className="absolute -right-6 bottom-0 text-[#B68F84]/15" size={180} strokeWidth={.5}/>
            <div className="relative max-w-[650px]">
              ...
              <button onClick={() => startGuided(suggestedRoutine)} className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#5B413A] px-5 py-3 text-[10px] font-medium text-white shadow-sm"><Play size={13}/>Start hair routine</button>
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            <S l="Current Style" v={last?.style ?? 'Not logged'} s={last ? `Since ${fmt(last.occurredAt)}` : 'Add your current style'}/>
            <S l="Next Wash" v={fmt(next)} s={`Current log rhythm ~${interval} days`}/>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
          ...
          <B t="Hair insight" subtitle="Glow combines your saved system with your actual logs.">
            <p className="glow-display text-[18px] leading-7 text-[#51413A]">{w.length > 1 ? `Your logged wash rhythm is about every ${interval} days. Your imported plan still anchors full resets to Sunday and maintenance to Thursday.` : 'Your imported plan anchors full resets to Sunday and maintenance to Thursday. As you log hair care, Glow will compare your real rhythm with the plan.'}</p>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <B t="Hair goals">{hg.length ? hg.map((g) => <div key={g.id} className="py-2"><div className="flex justify-between text-[10px]"><span>{g.title}</span><span>{g.progress}%</span></div><div className="mt-1.5 h-1.5 rounded-full bg-[#F1E7E3]"><div className="h-full rounded-full bg-[#C9727E]" style={{ width: `${g.progress}%` }}/></div></div>) : <Empty text="Add a hair goal and Glow will track it here."/>}</B>
      ...
      {tab === 'routines' ? <section className="space-y-4">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {masterRoutines.map((routine) => <div key={routine.id} className="rounded-[20px] border border-[#EEDFD9] bg-white p-5 shadow-[0_10px_40px_rgba(80,55,46,.04)]"><div className="flex items-start justify-between gap-3"><RoutineIcon kind={routine.icon}/><span className="rounded-full bg-[#FAF1EE] px-2.5 py-1 text-[8px] text-[#9A6E68]">{routine.cadence}</span></div><h2 className="glow-display mt-4 text-[20px] text-[#43362F]">{routine.name}</h2><p className="mt-1 min-h-[36px] text-[10px] leading-5 text-[#86756E]">{routine.subtitle}</p><div className="mt-4 space-y-2">{routine.steps.slice(0, 3).map((step, i) => <div key={step.title} className="flex gap-2 text-[9px] text-[#75665F]"><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[#F7EBE8] text-[8px] text-[#B16A74]">{i + 1}</span><span>{step.title}</span></div>)}</div><button onClick={() => startGuided(routine)} className="mt-5 inline-flex items-center gap-1.5 text-[10px] font-medium text-[#B2606E]">Open {routine.steps.length}-step guide <ChevronRight size={12}/></button></div>)}
        </div>
        {hrs.length ? <B t="Your live Glow routines" subtitle="These are routines already saved in your Glow database and remain fully editable.">{hrs.slice(0, 8).map((r) => <button key={r.id} onClick={() => setPlay(r)} className="flex w-full items-center justify-between rounded-[12px] px-3 py-2.5 text-left hover:bg-[#FAF7F5]"><span><b className="block text-[10.5px] font-medium">{r.name}</b><small className="text-[9px] text-[#92867E]">{routineSteps.filter((x) => x.routineId === r.id).length} saved steps</small></span><Play size={13}/></button>)}</B> : null}
      ...
      {tab === 'week' ? <section className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
        ...
        <div className="space-y-4">
          <B t="Cadence map"><Cadence label="Daily" detail="Gentle handling · moisture check · night protection"/><Cadence label="Every 1–2 days" detail="Hydration refresh: water + aloe, pea-size leave-in, 2–3 drops jojoba on ends"/><Cadence label="Every ~3 days" detail="Refresh curls · redo blend · refresh leave-out"/><Cadence label="Weekly" detail="Wash days · deep treatment · scalp reset"/><Cadence label="Monthly" detail="Wash wig/extensions · trim split ends if needed · clarify if buildup exists"/></B>
          <B t="Wash rhythm"><div className="grid grid-cols-2 gap-2"><MiniStat label="Sunday" value="Full reset"/><MiniStat label="Thursday" value="Repair + maintain"/><MiniStat label="Last wash" value={w[0] ? fmt(w[0].occurredAt) : '—'}/><MiniStat label="Forecast" value={fmt(next)}/></div></B>
        </div>
      </section> : null}
      ...
      {tab === 'products' ? <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
        <B t="Routine product shelf" subtitle="Products and ingredients explicitly named in your imported Hair system.">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {['Aloe vera + water','Leave-in conditioner','Jojoba oil','Lightweight serum','Argan oil','Coconut oil','Castor oil','Pumpkin seed oil','Rosemary oil','Grape seed oil','Onion oil','Heat protectant','Bond treatment','Hair mask','Pantene Volume','L’Oréal gloss shampoo'].map((p) => <div key={p} className="rounded-[14px] border border-[#F0E6E2] bg-[#FFFDFC] p-3"><span className="grid h-9 w-9 place-items-center rounded-[10px] bg-[#F7ECE8] text-[#B37370]"><Droplets size={13}/></span><p className="mt-2 text-[9px] leading-4 text-[#665650]">{p}</p></div>)}
          </div>
        </B>
      ...
      <details className="rounded-[18px] border border-[#EEE3DE] bg-white p-4">
        ...
        <form action={createHairLogAction} className="mt-3 grid gap-2 md:grid-cols-3"><input name="eventType" required placeholder="Wash, trim, treatment…" className={F}/><input name="style" placeholder="Style" className={F}/><input name="products" placeholder="Products" className={F}/><label className="flex items-center gap-2 text-[10px]"><input name="heatUsed" type="checkbox"/> Heat used</label><input name="nextAction" placeholder="Next action" className={F}/><button className="rounded-full bg-[#C9727E] px-4 py-2 text-[10px] text-white">Save log</button></form>
      </details>

      {guided ? <div className="fixed inset-0 z-[90] grid place-items-end bg-[#3C2D28]/25 p-3 backdrop-blur-[3px] sm:place-items-center"><div className="w-full max-w-[620px] overflow-hidden rounded-[28px] border border-white/70 bg-[#FFFDFC] shadow-[0_30px_100px_rgba(49,33,28,.25)]"><div className="border-b border-[#F0E3DE] bg-[linear-gradient(135deg,#F8E9E7,#FFFDFC)] p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[8px] uppercase tracking-[.16em] text-[#AE7378]">Guided Hair · {guided.cadence}</p><h2 className="glow-display mt-1 text-[25px] text-[#40332D]">{guided.name}</h2><p className="mt-1 text-[9px] text-[#8C7770]">Step {stepIndex + 1} of {guided.steps.length} · {guided.minutes}</p></div><button onClick={() => setGuided(null)} className="rounded-full border border-[#EADDD8] px-3 py-1.5 text-[9px] text-[#8F7770]">Close</button></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-[#C9727E] transition-all" style={{ width: `${((stepIndex + 1) / guided.steps.length) * 100}%` }}/></div></div><div className="p-5 sm:p-7"><div className="flex items-center gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#F6E3E5] glow-display text-[18px] text-[#A85E69]">{stepIndex + 1}</span><h3 className="glow-display text-[22px] text-[#433630]">{guided.steps[stepIndex].title}</h3></div><p className="mt-5 rounded-[16px] bg-[#FAF4F1] p-4 text-[11px] leading-6 text-[#6F5D56]">{guided.steps[stepIndex].detail}</p><label className="mt-5 flex cursor-pointer items-center gap-3 rounded-[14px] border border-[#F0E4DF] p-3 text-[10px] text-[#6D5C55]"><input type="checkbox" checked={!!completed[`${guided.id}-${stepIndex}`]} onChange={(e) => setCompleted((old) => ({ ...old, [`${guided.id}-${stepIndex}`]: e.target.checked }))}/><span>Mark this step complete</span></label><div className="mt-6 flex items-center justify-between gap-2"><button disabled={stepIndex === 0} onClick={() => setStepIndex((i) => Math.max(0, i - 1))} className="rounded-full border border-[#E8DAD5] px-4 py-2.5 text-[10px] disabled:opacity-35">Back</button><button onClick={() => { if (stepIndex >= guided.steps.length - 1) { setGuided(null); setStepIndex(0); } else setStepIndex((i) => i + 1); }} className="rounded-full bg-[#5B413A] px-5 py-2.5 text-[10px] font-medium text-white">{stepIndex >= guided.steps.length - 1 ? 'Finish routine ✦' : 'Next step'}</button></div></div></div></div> : null}
