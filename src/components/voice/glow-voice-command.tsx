...
export function GlowVoiceCommand(){
  ...
  async function runCommand(){
    ...
    if(/morning brief|prepare me for today|what do i need to know today/.test(normalized)){router.push('/briefings?view=morning');setStatus('Opening your data-backed Morning Brief.');setText('');setTimeout(()=>setOpen(false),450);return;}
    if(/night brief|evening brief|prepare me for tomorrow|how did today go|finish my day/.test(normalized)){router.push('/briefings?view=night');setStatus('Opening your reflective Night Brief.');setText('');setTimeout(()=>setOpen(false),450);return;}
    const searchMatch=command.match(/^(?:search(?: for)?|find)\s+(.+)/i);
    if(searchMatch?.[1]){router.push(`/search?q=${encodeURIComponent(searchMatch[1])}`);setStatus('Searching your authenticated Glow OS records.');setText('');setTimeout(()=>setOpen(false),450);return;}
