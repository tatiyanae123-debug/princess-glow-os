...
import { AlertTriangle, CheckCircle2, Mic, Search, Send, ShieldCheck, X } from 'lucide-react';
...
function personalRoute(text:string){
  ...
  if(/good morning|just woke up|morning brief|plan my day|what should i do today/.test(value))return'/briefings/morning';
  if(/what should i do next|what('?s| is) next/.test(value))return'/today';
...
export function GlowVoiceCommand(){
  ...
  function startListening(){setStatus('');const browser=window as Window&{SpeechRecognition?:RecognitionCtor;webkitSpeechRecognition?:RecognitionCtor};const Ctor=browser.SpeechRecognition??browser.webkitSpeechRecognition;if(!Ctor){setStatus('Voice recognition is not available in this browser. Type your command instead.');return;}const recognition=new Ctor();recognition.continuous=false;recognition.interimResults=false;recognition.lang='en-US';recognition.onresult=(event)=>{setText(Array.from(event.results).map(result=>result[0]?.transcript??'').join(' ').trim());setListening(false);};recognition.onend=()=>setListening(false);recognition.onerror=(event)=>{setListening(false);setStatus(`Glow could not hear that clearly (${event.error}).`);};recognitionRef.current=recognition;setListening(true);recognition.start();}
  function stopListening(){recognitionRef.current?.stop();setListening(false);}

  async function runCommand(){const command=text.trim();if(!command||pending)return;const lower=command.toLowerCase();const personal=personalRoute(command);if(personal){router.push(personal);setOpen(false);setText('');return;}const destination=navigationTarget(command);if(destination){router.push(destination);setOpen(false);setText('');return;}if(/^(new task|add a task|add task)$/.test(lower)){document.dispatchEvent(new CustomEvent('glow:quick-add',{detail:{module:'task'}}));setOpen(false);return;}if(/^(new event|add event|add an event)$/.test(lower)){document.dispatchEvent(new CustomEvent('glow:quick-add',{detail:{module:'event'}}));setOpen(false);return;}if(/plan tomorrow/.test(lower)){router.push('/planning?view=tomorrow');setOpen(false);return;}if(/search/.test(lower)&&/doctor|note|notes/.test(lower)){router.push(`/brain?mode=search&q=${encodeURIComponent(command)}`);setOpen(false);return;}setPending(true);setStatus('Glow is thinking across your life system…');try{const response=await fetch('/api/voice/command',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({text:command,sourceRoute:pathname,risk})});const payload=await response.json() as {ok?:boolean;message?:string};if(!response.ok||!payload.ok)throw new Error(payload.message||'Glow could not run that command.');setStatus(payload.message||'Done.');setText('');router.refresh();}catch(error){setStatus(error instanceof Error?error.message:'Glow could not run that command.');}finally{setPending(false);}}

  if(!open)return null;
  return <div className="fixed inset-0 z-[110] flex items-start justify-center bg-[#1b1520]/20 px-3 pt-[max(8vh,env(safe-area-inset-top))] backdrop-blur-[8px]" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false);}}>
    <section className="max-h-[min(780px,84vh)] w-full max-w-[680px] overflow-y-auto rounded-[28px] border border-white/80 bg-white/92 shadow-[0_34px_110px_rgba(75,58,91,.24)] backdrop-blur-2xl">
      <div className="relative overflow-hidden border-b border-[#EFEAF1] px-5 py-6 sm:px-7">
        ...
        <div className="relative flex items-start gap-4">
          <GlowOrb state={orbState} size={68} className="mt-1" />
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-[#8E8096]">Glow presence</p>
            <h2 className="glow-display mt-1 text-[30px] leading-tight text-[#1C1C1E]">Talk to your life.</h2>
            <p className="mt-2 max-w-[480px] text-[13px] leading-5 text-[#6E6E73]">Speak naturally. Glow now knows your routines, habits, 2026 workout plan, briefs and visual cards as part of the same action layer.</p>
          </div>
      ...
      <div className="space-y-4 p-5 sm:p-7">
        <div className="relative"><Search className="absolute left-4 top-4 text-[#9A9A9F]" size={17}/><textarea autoFocus value={text} onChange={event=>setText(event.target.value)} onKeyDown={event=>{if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();void runCommand();}}} rows={3} placeholder="I just woke up. What should I do? Show my Sunday reset card. What’s my workout?…" className="w-full resize-none rounded-[18px] border border-[#E4E0E6] bg-[#FCFBFC] py-3.5 pl-11 pr-4 text-[14px] leading-6 text-[#1C1C1E] outline-none transition focus:border-[#C9B6D0] focus:ring-4 focus:ring-[#EFE8F3]/60"/></div>
        ...
        {status?<div role="status" className="flex items-center gap-3 rounded-[14px] bg-[#F8F5F9] px-3 py-3 text-[12px] leading-5 text-[#5F5962]"><GlowOrb state={orbState} size={28}/><span>{status}</span></div>:null}
        <button type="button" disabled={!text.trim()||pending} onClick={()=>void runCommand()} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-[14px] bg-[#1C1C1E] px-4 text-[13px] font-semibold text-white transition hover:bg-black disabled:opacity-40"><Send size={14}/>{pending?'Glow is working…':risk==='high'?'Review action':'Do it'}</button>
        <div className="grid gap-2 sm:grid-cols-3">{['Good morning — plan my day','What should I do next?','Show today’s habits','Start my morning routine','What’s my workout today?','Show my Sunday reset card'].map(example=><button type="button" key={example} onClick={()=>setText(example)} className="rounded-[13px] border border-[#ECE8ED] bg-white px-3 py-2.5 text-left text-[12px] leading-5 text-[#59545B] transition hover:bg-[#FBF9FC]">{example}</button>)}</div>
      </div>
    </section>
  </div>;
}
