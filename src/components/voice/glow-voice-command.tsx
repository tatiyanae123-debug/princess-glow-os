  ...
  return <div className="fixed inset-0 z-[110] flex items-start justify-center bg-[#1b1520]/20 px-3 pt-[max(8vh,env(safe-area-inset-top))] backdrop-blur-[8px]" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false);}}>
    <section className="max-h-[min(780px,84vh)] w-full max-w-[680px] overflow-y-auto rounded-[28px] border border-white/80 bg-white/92 shadow-[0_34px_110px_rgba(75,58,91,.24)] backdrop-blur-2xl">
      <div className="relative overflow-hidden border-b border-[#EFEAF1] px-5 py-6 sm:px-7">
        ...
        <div className="relative flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 max-w-[480px] text-[13px] leading-5 text-[#6E6E73]">Speak naturally. Glow can understand the room you are in, route information, create actions and keep your system connected.</p>
          </div>
      ...
      <div className="space-y-4 p-5 sm:p-7">
        <div className="relative"><Search className="absolute left-4 top-4 text-[#9A9A9F]" size={17}/><textarea autoFocus value={text} onChange={event=>setText(event.target.value)} onKeyDown={event=>{if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();void runCommand();}}} rows={3} placeholder="I just woke up. What should I do? Move my workout. Start my Sunday reset…" className="w-full resize-none rounded-[18px] border border-[#E4E0E6] bg-[#FCFBFC] py-3.5 pl-11 pr-4 text-[14px] leading-6 text-[#1C1C1E] outline-none transition focus:border-[#C9B6D0] focus:ring-4 focus:ring-[#EFE8F3]/60"/></div>
        ...
        <div className="grid gap-2 sm:grid-cols-3">{['Good morning — plan my day','What should I do next?','Show today’s habits','Start my morning routine','Move my workout','Plan my Sunday reset'].map(example=><button type="button" key={example} onClick={()=>setText(example)} className="rounded-[13px] border border-[#ECE8ED] bg-white px-3 py-2.5 text-left text-[12px] leading-5 text-[#59545B] transition hover:bg-[#FBF9FC]">{example}</button>)}</div>
      </div>
    </section>
  </div>;
}
