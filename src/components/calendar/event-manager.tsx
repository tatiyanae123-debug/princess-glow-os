            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  {event.source === 'google_calendar' && (
                    <span className="rounded-full px-2 py-1 text-[10px]" style={{ background: 'var(--glow-accent-soft)', color: 'var(--glow-accent)' }}>
                      Google
                    </span>
                  )}
                  {event.editable && (
              ...
              {event.source === 'google_calendar' && (
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button type="button" variant="secondary" disabled={convert.isPending} onClick={() => convert.run({ eventId: event.id })}>
                    Convert to Task
                  </Button>
                  <Button type="button" variant="ghost" disabled title="Coming in a future AI Concierge phase">
                    <Sparkles size={13} /> Build My Day Around This Schedule
                  </Button>
                </div>
              )}
            </div>
