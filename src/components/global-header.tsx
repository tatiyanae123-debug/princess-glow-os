...
export function GlobalHeader() {
  ...
  return <header className="glow-global-header sticky top-0 z-40 flex h-[54px] items-center justify-end border-b border-[#F0F0F0] bg-white/96 px-4 sm:px-5 lg:px-6">
    <div className="flex items-center gap-1.5">
      ...
      <button type="button" onClick={() => openGlow()} className="glow-header-action" aria-label="Speak to Glow"><Mic2 size={16}/></button>
      <button type="button" onClick={() => openGlow('Create ')} className="glow-header-action glow-header-create" aria-label="Create with Glow"><Plus size={17}/></button>
