...
Align the becoming.'}</blockquote><MoreHorizontal/>
          ...
          <div className={styles.projectMain} data-view={view}>
            ...
            {view === 'list' ? <div className={styles.projectListView}><article><h3>Actions</h3>{projectTasks.map((task) => <p key={task.id}><Circle/> {task.title}<span>{task.status.replace(/_/g, ' ')}</span></p>)}</article><article><h3>Notes & decisions</h3>{[...notes, ...decisions.map((item, index) => { const record = item as JsonMap; return { id: `d-${index}`, title: asString(record.title), summary: asString(record.summary), category: 'decision', source: asString(record.actor), sourceDate: asString(record.date), relatedProjectId: selected?.id ?? null }; })].map((item) => <p key={item.id}>{item.title}<small>{item.summary}</small></p>)}</article></div> : null}
          ...
          <div className={styles.projectBottom}>
            ...
            <article className={styles.decisions}><header>Recent Decisions <button type="button" onClick={() => openGlow({ room: 'Plan · Projects', project: selected?.title ?? '', intent: 'Show all project decisions' })}>View all <ChevronRight/></button></header>{decisions.length ? decisions.slice(0, 3).map((decision, index) => <p key={index}><Check/><span><strong>{asString((decision as JsonMap).title) || 'Decision'}</strong><small>{shortDate(asString((decision as JsonMap).date) || null)}{asString((decision as JsonMap).actor) ? ` · ${asString((decision as JsonMap).actor)}` : ''}</small></span></p>) : <p className={styles.empty}>No recorded decisions with provenance.</p>}</article>
          </div>
        </div>
      </section>
    </PlanInstrumentChrome>
