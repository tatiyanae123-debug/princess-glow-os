...
export function Batch8InterviewModeView({ events = [], tasks = [] }: { events?: InterviewEvent[]; tasks?: Task[] }) {
  ...
  return <div className="b8-page b8-interview">
    <Head eyebrow="6. INTERVIEW MODE" title="Interview Mode" sub="Prepare. Practice. Perfect." action={<Link className="b8-soft-btn" href="/work/interviews">Exit Mode</Link>} />
    ...
    <div className="b8-two-col">
      <Card><h2>Preparation Checklist</h2><div className="b8-check-list">{prep.length ? prep.map((task)=><Link key={task.id} href={`/tasks?taskId=${encodeURIComponent(task.id)}`}><Circle size={10}/><span>{task.title}</span></Link>) : ['Research company','Review role & responsibilities','Practice common questions','Prepare questions to ask','Choose what to bring'].map((label)=><div key={label}><Circle size={10}/><span>{label}</span></div>)}</div>{!prep.length?<p className="b8-footnote">Suggestions only. They are not marked complete until you create or complete real tasks.</p>:<p className="b8-footnote">Open preparation tasks are shown with an empty circle so Glow does not imply they are already complete.</p>}</Card>
...
export function Batch8WorkoutModeView({ sessions = [] }: { sessions?: FitnessSession[] }) {
  ...
  return <div className="b8-page b8-workout">
    <Head eyebrow="7. WORKOUT MODE" title="Workout Mode" sub="Move with intention. Track progress." action={<Link className="b8-soft-btn" href="/fitness">Exit Mode</Link>} />
    <section className="b8-workout-hero">
      ...
      <div className="b8-workout-copy"><p className="b8-kicker">{latest ? 'Latest recorded session' : 'Ready when you are'}</p><h2>{latest?.workoutType ?? 'Start your next workout'}</h2><p>{latest ? `${latest.durationMinutes ?? 'Duration not saved'}${latest.durationMinutes ? ' min' : ''} · ${latest.equipment || 'Equipment not saved'}` : 'Use Fitness to log a real workout. This mode will reflect your latest recorded session.'}</p><Link className="b8-primary" href="/fitness"><Play size={12}/>{latest ? 'Open Fitness' : 'Start in Fitness'}</Link></div>
    </section>
    <div className="b8-metrics">{[
      ['Recent Sessions', sessions.length],
      ['Latest Duration', latest?.durationMinutes ? `${latest.durationMinutes} min` : '—'],
      ['Latest Energy', latest?.energy != null ? `${latest.energy}/5` : '—'],
    ...
    <div className="b8-two-col b8-workout-lower">
      <Card><h2>Latest Workout Details</h2>{latest ? <div className="b8-session-details"><p><Dumbbell size={12}/><span>{latest.workoutType}</span></p><p><CalendarDays size={12}/><span>{latest.occurredAt.toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</span></p><p><Sparkles size={12}/><span>Soreness {latest.soreness != null ? `${latest.soreness}/5` : 'not logged'}</span></p></div> : <p className="b8-muted">No fitness session has been recorded yet.</p>}{latest?.notes?<div className="b8-note"><NotebookPen size={12}/><p>{latest.notes}</p></div>:null}</Card>
      <Card className="b8-body-card"><h2>Body Reference</h2><EditableRoomImage slot="batch8:workout:body" label="Body reference" fallbackUrl={photo('photo-1571019614242-c5c5dee9f50b')} className="b8-body-image"/><p className="b8-footnote">Reference image only. Glow does not infer trained muscle groups unless they are stored in your workout data.</p></Card>
    </div>
