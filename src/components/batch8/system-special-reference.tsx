...
export function Batch8InterviewModeView({events=[],tasks=[]}:{events?:InterviewEvent[];tasks?:Task[]}){
 ...
 return <div className="b8-page b8-interview">
  ...
  <div className="b8-two-col"><Card><h2>Preparation Checklist</h2><div className="b8-check-list">{prep.length?prep.map(task=><Link key={task.id} href={`/tasks?taskId=${encodeURIComponent(task.id)}`}><Circle size={10}/><span>{task.title}</span></Link>):['Research the company','Review job description','Practice common questions','Prepare questions to ask','Outfit & logistics'].map(label=><div key={label}><Circle size={10}/><span>{label}</span></div>)}</div><p className="b8-footnote">Only saved tasks are treated as real preparation records.</p></Card><Card><h2>Practice Questions</h2><div className="b8-question-list">{['Tell me about yourself.','Why this company?','What are your strengths?','How do you handle stress?','Where do you see yourself growing?'].map(q=><Link href="/notes" key={q}><MessageCircleQuestion size={11}/><span>{q}</span></Link>)}</div></Card></div>
...
export function Batch8WorkoutModeView({sessions=[]}:{sessions?:FitnessSession[]}){
 ...
 return <div className="b8-page b8-workout">
  ...
  <nav className="b8-tabs" aria-label="Workout categories"><span className="active">Strength</span><span>Pilates</span><span>Cardio</span><span>Yoga</span><span>Stretch</span></nav>
