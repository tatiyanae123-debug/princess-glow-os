...
function Side({name,image,onClose}:{name:string;image?:string|null;onClose?:()=>void}){
 ...
 return <aside className="ir-sidebar">
  <div className="ir-brand"><Sparkles size={18}/><span>Glow OS</span>{onClose?<button onClick={onClose} aria-label="Close navigation"><X size={16}/></button>:null}</div>
  ...
  <div className="ir-side-bottom"><Link href="/settings?section=profile" className="ir-profile">{image?<img src={image} alt="Profile"/>:<span>{name.slice(0,1).toUpperCase()}</span>}<div><strong>{name}</strong><small>View Profile</small></div></Link><Link href="/settings" className="ir-more"><Settings size={15}/></Link></div>
 </aside>
}
...
export function ImmersiveTopControls({searchHref='/search'}:{searchHref?:string}){
 ...
 return <div className="ir-top-controls"><Link href={searchHref} aria-label="Search"><Search size={16}/></Link><Link href="/memory" aria-label="Memory"><NotebookTabs size={16}/></Link><button onClick={customize} aria-label="Customize"><Settings size={16}/></button></div>
}
