...
import { Check, ChefHat, Clock3, Refrigerator, ShoppingBasket, Sparkles, Utensils } from 'lucide-react';
...
export function FoodRoom(){
  ...
  return <div className="space-y-4">
    ...
    {tab==='Recipes'&&<section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{recipes.map(([name,time,tags,ingredients],index)=><article key={name} className="paper-card overflow-hidden"><div className={`h-28 ${index%3===0?'bg-[linear-gradient(135deg,#dce5d7,#ead4c8)]':index%3===1?'bg-[linear-gradient(135deg,#e8d6cb,#d2ddc9)]':'bg-[linear-gradient(135deg,#ead4d6,#eee6d3)]'} flex items-center justify-center`}><Utensils size={34} strokeWidth={.8} className="text-white/80"/></div><div className="p-4"><p className="glow-display text-[16px] text-[#4a443f]">{name}</p><p className="mt-1 text-[8px] text-[#9b8f87]">{time} · {tags}</p><p className="mt-3 text-[9px] leading-4 text-[#6d655f]">{ingredients}</p><div className="mt-3 flex gap-2"><button className="text-[8px] font-semibold text-[#936d6e]">View recipe</button><button className="text-[8px] text-[#8a8b80]">Add to week</button></div></div></article>)}</section>}
    ...
    {tab==='Pantry'&&<section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{[['Fridge','Greek yogurt · strawberries · spinach'],['Freezer','Frozen fruit · stir-fry vegetables'],['Pantry','Quinoa · oats · rice · lentils · beans'],['Always Keep','Collagen · frozen fruit · strawberries · honey']].map(([name,content])=><article key={name} className="paper-card p-4"><Refrigerator size={18} className="text-[#8d9c83]"/><h3 className="glow-display mt-3 text-[16px] text-[#4f5048]">{name}</h3><p className="mt-2 text-[9px] leading-5 text-[#746e67]">{content}</p><button className="mt-3 text-[8px] font-semibold text-[#956f6e]">Update inventory →</button></article>)}</section>}
