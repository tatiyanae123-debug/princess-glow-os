...
function taskMeta(task:TaskLite|undefined,index:number){
  ...
  if(raw) return raw.replace(/minutes?/i,'MIN').replace(/hours?/i,'HR');
  if(index===1) return '60 MIN';
  return 'Today';
}

function titleSize(title:string, base=1.68, min=1.05){
  const amount=Math.max(0,title.length-20);
  return `${Math.max(min,base-amount*.017)}cqw`;
}
...
function Hotspot({label,style,onClick}:{label:string;style:CSSProperties;onClick:()=>void}){
  return <button
    ...
    onClick={onClick}
    style={style}
    className="absolute z-30 rounded-[1cqw] bg-transparent outline-none transition hover:bg-white/[.055] focus-visible:ring-2 focus-visible:ring-[#bd948d] focus-visible:ring-offset-2 focus-visible:ring-offset-transparent"
  />;
}
...
function Modal({children,onClose}:{children:ReactNode;onClose:()=>void}){
  return <div className="absolute inset-0 z-50 grid place-items-center bg-[#6f5149]/10 p-4 backdrop-blur-[3px] sm:p-6" onMouseDown={onClose}>
    <section
      role="dialog"
      aria-modal="true"
      onMouseDown={event=>event.stopPropagation()}
      className="relative max-h-[84dvh] w-[min(92vw,590px)] overflow-y-auto rounded-[28px] border border-white/80 bg-[linear-gradient(145deg,rgba(255,252,249,.96),rgba(244,226,220,.91))] p-6 text-[#312522] shadow-[0_24px_90px_rgba(92,61,52,.22),inset_0_1px_0_rgba(255,255,255,.96)] sm:p-7"
    >
      <button type="button" onClick={onClose} aria-label="Close" className="absolute right-4 top-4 grid h-10 w-10 place-items-center rounded-full border border-white/70 bg-white/55 text-xl shadow-sm">×</button>
      {children}
    </section>
  </div>;
}

function emitGlowOpen(detail: {prompt?:string;activeObject?:string;listen?:boolean} = {}) {
  window.dispatchEvent(new CustomEvent('glow:open',{detail}));
}

function emitNavigate(href:string,label:string){
  window.dispatchEvent(new CustomEvent('glow:navigate',{detail:{href,label}}));
}

function emitRipple(label:string){
  window.dispatchEvent(new CustomEvent('glow:ripple',{detail:{label}}));
}
...
export function TodayLivingCenter({tasks,events,routines,energy,mood,sleepHours,glowMessage}:Props){
  ...
  useEffect(()=>{
    ...
    if(!next.some(task=>task.id===selectedTaskId)) setSelectedTaskId(next[0].id);
  ...
  useEffect(()=>{
    if(focusSeconds===0) {
      ...
      setReceipt('Focus block complete. The moment released its light back into Today.');
      emitRipple('focus-complete');
    }
  },[focusSeconds]);
  ...
  useEffect(()=>{
    const ripple=()=>setRippleKey(value=>value+1);
    window.addEventListener('glow:ripple',ripple);
    return ()=>window.removeEventListener('glow:ripple',ripple);
  },[]);

  useEffect(()=>{
    if(!receipt) return;
    const timer=window.setTimeout(()=>setReceipt(''),4200);
  ...
  function chooseTask(task:TaskLite){
    setSelectedTaskId(task.id);
    setPanel('focus');
    setReceipt(`${task.title} moved into focus without leaving Today.`);
    emitRipple('focus-task');
  }

  function toggleRoutine(routine:RoutineLite){
    setCompletedRoutineIds(current=>{
      const done=current.includes(routine.id);
      setReceipt(done?`${routine.name} returned to the active pathway.`:`${routine.name} completed. Its light settled into Today.`);
      emitRipple(done?'routine-reopen':'routine-complete');
      return done?current.filter(id=>id!==routine.id):[...current,routine.id];
    });
  }

  function previewReplan(){
    const preview=[...displayTasks].sort((a,b)=>{
      const rank=(priorityRank[b.priority]??0)-(priorityRank[a.priority]??0);
      if(rank!==0)return rank;
      const aSoon=/today|overdue/i.test(a.dueLabel??'')?1:0;
      const bSoon=/today|overdue/i.test(b.dueLabel??'')?1:0;
      return bSoon-aSoon;
    });
    setReplanPreview(preview);
    setPanel('replan');
    emitRipple('replan-preview');
  }

  function applyReplanPreview(){
    if(replanPreview.length)setDisplayTasks(replanPreview);
    setPanel(null);
    setReceipt('Today reorganized around priority and capacity. No external calendar change was made without approval.');
    emitRipple('replan-applied');
  }

  function openMoment(index:number){
    setMomentIndex(index);
    setPanel('moment');
    setReceipt(`${schedule[index].label} came into focus while Now stayed anchored.`);
    emitRipple('moment-focus');
  }

  function navigate(href:string,label:string){
    emitNavigate(href,label);
  }
  ...
  return <main className="h-[100dvh] w-full overflow-hidden bg-[#eadfd8] text-[#312522]">
    <style>{`
      ...
      .living-ripple{animation:livingRipple .82s cubic-bezier(.15,.72,.18,1) both}
      .today-receipt{animation:receiptIn .24s ease-out both}
      .portrait-pan{overflow:hidden}
      @media (orientation:portrait){
        .portrait-pan{overflow-x:auto;overflow-y:hidden;scrollbar-width:none;overscroll-behavior-x:contain;-webkit-overflow-scrolling:touch}
      ...
      @media (prefers-reduced-motion:reduce){.living-ripple,.today-receipt{animation:none!important}}
    `}</style>
    ...
    <div className="portrait-pan flex h-full w-full items-center justify-center bg-[#eadfd8]">
      <div className="glow-stage relative shrink-0 overflow-hidden" style={{width:'min(100vw, calc(100dvh * 4 / 3))',height:'min(100dvh, calc(100vw * 3 / 4))',containerType:'inline-size'}}>
        ...
        <div aria-live="polite" className="absolute z-20 flex flex-col justify-center" style={{left:'3.9%',top:'5.4%',width:'30%',height:'8.1%',background:'linear-gradient(90deg,rgba(255,249,246,.94) 0%,rgba(255,249,246,.83) 72%,rgba(255,249,246,0) 100%)',WebkitMaskImage:'linear-gradient(90deg,#000 0%,#000 78%,transparent 100%)',maskImage:'linear-gradient(90deg,#000 0%,#000 78%,transparent 100%)',backdropFilter:'blur(2px)',WebkitBackdropFilter:'blur(2px)'}}>
        ...
        {displayTasks.slice(0,3).map((task,index)=>{
          ...
          return <DataLine key={task.id} left={8.1} top={tops[index]} width={16.2} height={2.75}><span className="min-w-0 flex-1 truncate font-serif" style={{fontSize:'.72cqw'}}>{task.title}</span><span className="ml-[.45cqw] shrink-0" style={{fontSize:'.57cqw'}}>{taskMeta(task,index)}</span></DataLine>;
        })}
        ...
        {topThree.map((task,index)=>{
          const lefts=[6.5,23.2,39.1]; const widths=[12.6,12.5,13.1];
          return <DataLine key={`priority-${task.id}`} left={lefts[index]} top={80.55} width={widths[index]} height={3.0}><span className="truncate font-serif" style={{fontSize:'1.05cqw'}}>{task.title}</span></DataLine>;
        })}
        {topThree.map((task,index)=>{
          const lefts=[6.5,23.2,39.1];
          return <DataLine key={`impact-${task.id}`} left={lefts[index]} top={85.9} width={9.2} height={2.2}><span style={{fontSize:'.54cqw'}}>Impact: {priorityLabel(task.priority)}</span></DataLine>;
        })}

        {schedule.map((item,index)=>{
          const topSets=[{time:14.1,title:16.45,note:21.0},{time:30.5,title:32.95,note:37.65},{time:47.0,title:49.25,note:54.0},{time:62.0,title:64.2,note:0}];
          const p=topSets[index];
          return <div key={item.label}>
            <DataLine left={82.15} top={p.time} width={11.1} height={2.2}><span style={{fontSize:'.59cqw'}}>{item.time}</span></DataLine>
            <DataLine left={82.15} top={p.title} width={12.1} height={3.2}><span className="truncate font-serif" style={{fontSize:'1.03cqw'}}>{item.title}</span></DataLine>
            {p.note>0&&<DataLine left={82.15} top={p.note} width={12.2} height={2.2}><span className="truncate" style={{fontSize:'.56cqw'}}>{item.note}</span></DataLine>}
          </div>;
        })}

        {liveRoutines.slice(0,3).map((routine,index)=>{
          const tops=[78.55,82.7,86.85]; const done=completedRoutineIds.includes(routine.id);
          return <DataLine key={routine.id} left={57.75} top={tops[index]} width={13.4} height={2.55}><span className={`truncate font-serif ${done?'line-through opacity-45':''}`} style={{fontSize:'.66cqw'}}>{routine.name}</span></DataLine>;
        })}
        ...
        {topThree.map((task,index)=>{
          const positions=[[4.1,74.7,16.4,16.2],[20.6,74.7,16.1,16.2],[36.8,74.7,16.4,16.2]] as const;
          const [left,top,width,height]=positions[index];
          return <Hotspot key={`priority-hotspot-${task.id}`} label={`Focus ${task.title}`} style={hotspot(left,top,width,height)} onClick={()=>chooseTask(task)} />;
        })}

        {liveRoutines.slice(0,3).map((routine,index)=>{
          const tops=[77.0,81.2,85.3];
          return <Hotspot key={`routine-hotspot-${routine.id}`} label={`${completedRoutineIds.includes(routine.id)?'Reopen':'Complete'} ${routine.name}`} style={hotspot(54.4,tops[index],20.9,4.0)} onClick={()=>toggleRoutine(routine)} />;
        })}
        ...
        {panel==='search'&&<Modal onClose={()=>setPanel(null)}>
          <p className="text-xs font-semibold tracking-[.2em]">SEARCH GLOW OS</p><h2 className="mt-2 font-serif text-3xl">Cast light across your world.</h2>
          <input autoFocus value={searchText} onChange={event=>setSearchText(event.target.value)} placeholder="Tasks, moments, routines…" className="mt-6 w-full rounded-2xl border border-[#d8c2bb] bg-white/60 px-4 py-3 outline-none focus:ring-2 focus:ring-[#cba6a0]" />
          <div className="mt-4 space-y-2">{searchText&&!searchResults.length&&<p className="text-sm opacity-70">No matching live items yet.</p>}{searchResults.map((result,index)=><button key={`${result.kind}-${index}`} type="button" onClick={()=>{setPanel(null);navigate(result.href,result.activeObject);}} className="block w-full rounded-xl bg-white/42 px-4 py-3 text-left transition hover:bg-white/70"><span className="mr-2 text-[10px] font-semibold tracking-[.15em] opacity-60">{result.kind.toUpperCase()}</span>{result.title}</button>)}</div>
        </Modal>}

        {panel==='focus'&&<Modal onClose={()=>setPanel(null)}>
          <p className="text-xs font-semibold tracking-[.2em]">LIVE MOMENT · NOW</p><h2 className="mt-2 font-serif text-3xl">{selectedTask.title}</h2><p className="mt-2 text-sm opacity-65">High focus · Creative work</p>
          <div className="mt-6 flex items-center gap-6"><button type="button" onClick={()=>setFocusRunning(value=>!value)} className="grid h-28 w-28 shrink-0 place-items-center rounded-full border-2 border-[#d8bdb5] bg-white/45 shadow-[inset_0_0_30px_rgba(255,255,255,.8)]"><div><div className="font-serif text-4xl">{focusMinutes}</div><div className="text-[10px] tracking-[.18em]">MIN</div></div></button><div><p className="font-serif italic text-[#9e766e]">This is your moment.</p><p className="mt-3 text-sm leading-relaxed opacity-70">The current object stays tied to Today. Start, pause, or move deeper without losing the timeline around it.</p></div></div>
          <div className="mt-6 flex flex-wrap gap-2"><button type="button" onClick={()=>setFocusRunning(value=>!value)} className="rounded-full bg-[#3d302c] px-5 py-2.5 text-sm text-white">{focusRunning?'Pause focus':'Start focus'}</button><button type="button" onClick={()=>navigate('/focus',selectedTask.title)} className="rounded-full border border-[#d5bdb6] bg-white/45 px-5 py-2.5 text-sm">Expand into Focus Studio</button><button type="button" onClick={()=>emitGlowOpen({prompt:`Help me focus on ${selectedTask.title}`,activeObject:selectedTask.title})} className="rounded-full border border-[#d5bdb6] bg-white/45 px-5 py-2.5 text-sm">Ask Glow</button></div>
        </Modal>}

        {panel==='what-now'&&<Modal onClose={()=>setPanel(null)}>
          <p className="text-xs font-semibold tracking-[.2em]">WHAT NOW?</p><h2 className="mt-2 font-serif text-3xl">Your next right three.</h2>
          <div className="mt-5 space-y-3">{displayTasks.slice(0,3).map((task,index)=><button key={task.id} type="button" onClick={()=>chooseTask(task)} className="flex w-full items-center gap-3 rounded-2xl bg-white/45 px-4 py-3 text-left transition hover:bg-white/70"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#ecd9d2]">{index+1}</span><span className="min-w-0 flex-1"><span className="block font-serif text-lg">{task.title}</span><span className="text-xs opacity-60">{taskMeta(task,index)} · {priorityLabel(task.priority)} impact</span></span></button>)}</div>
          <button type="button" onClick={()=>emitGlowOpen({prompt:'Show me what I need to do next.',activeObject:'What Now'})} className="mt-5 rounded-full border border-[#d4bbb4] bg-white/50 px-5 py-2.5 text-sm">Ask Glow to choose with me</button>
        </Modal>}

        {panel==='energy'&&<Modal onClose={()=>setPanel(null)}>
          <p className="text-xs font-semibold tracking-[.2em]">ENERGY & CAPACITY</p><h2 className="mt-2 font-serif text-3xl">How much can today hold?</h2>
          <div className="mt-5 flex flex-col items-center gap-6 sm:flex-row"><div className="grid h-32 w-32 shrink-0 place-items-center rounded-full border-2 border-[#d9bdb5] bg-white/40 text-center"><div><div className="font-serif text-5xl">{capacity}</div><div className="font-serif">Radiant</div></div></div><div className="w-full flex-1 space-y-3">{[['Mental',capacity],['Emotional',emotional],['Physical',physical],['Creative',creative]].map(([label,value])=><div key={String(label)}><div className="mb-1 flex justify-between text-xs"><span>{label}</span><span>{value}%</span></div><div className="h-1 rounded-full bg-[#ddcec9]"><div className="h-full rounded-full bg-[#cda39c]" style={{width:`${value}%`}}/></div></div>)}</div></div>
          {sleepHours!=null&&<p className="mt-5 text-sm opacity-70">Last recorded sleep: {sleepHours} hours.</p>}<button type="button" onClick={()=>emitGlowOpen({prompt:'Replan the rest of today around my current capacity.',activeObject:'Energy & Capacity'})} className="mt-5 rounded-full border border-[#d4bbb4] bg-white/50 px-5 py-2.5 text-sm">Ask Glow to adapt the day</button>
        </Modal>}

        {panel==='priorities'&&<Modal onClose={()=>setPanel(null)}>
          <p className="text-xs font-semibold tracking-[.2em]">TOP 3 PRIORITIES</p><h2 className="mt-2 font-serif text-3xl">What deserves your light.</h2>
          <div className="mt-5 grid gap-3 sm:grid-cols-3">{topThree.map((task,index)=><button type="button" key={task.id} onClick={()=>chooseTask(task)} className="rounded-2xl bg-white/42 p-4 text-left transition hover:bg-white/70"><p className="text-[10px] tracking-[.15em] opacity-60">{['CREATE','CARE','PLAN'][index]}</p><p className="mt-2 font-serif text-xl">{task.title}</p><p className="mt-2 text-xs opacity-60">Impact: {priorityLabel(task.priority)}</p></button>)}</div>
        </Modal>}

        {panel==='routines'&&<Modal onClose={()=>setPanel(null)}>
          <p className="text-xs font-semibold tracking-[.2em]">ROUTINES DUE NOW</p><h2 className="mt-2 font-serif text-3xl">Move through the pathway.</h2>
          <div className="mt-5 divide-y divide-[#d9c8c2]">{liveRoutines.slice(0,5).map((routine,index)=>{const done=completedRoutineIds.includes(routine.id);return <button key={routine.id} type="button" onClick={()=>toggleRoutine(routine)} className="flex w-full items-center gap-3 py-3 text-left"><span className={`min-w-0 flex-1 font-serif text-lg ${done?'line-through opacity-45':''}`}>{routine.name}</span><span className="text-xs opacity-60">{[5,10,7,12,8][index]??8} MIN</span><span className={`grid h-5 w-5 place-items-center rounded-full border border-[#a9857d] ${done?'bg-[#9fa88e] text-white':''}`}>{done?'✓':''}</span></button>;})}</div>
          <button type="button" onClick={()=>navigate('/routines','Routines')} className="mt-5 rounded-full border border-[#d4bbb4] bg-white/50 px-5 py-2.5 text-sm">Open guided Routine Studio</button>
        </Modal>}

        {panel==='replan'&&<Modal onClose={()=>setPanel(null)}>
          <p className="text-xs font-semibold tracking-[.2em]">REPLAN MY DAY</p><h2 className="mt-2 font-serif text-3xl">A reversible preview, not a silent rewrite.</h2><p className="mt-3 text-sm leading-relaxed opacity-70">Glow gathered the most important unfinished work toward Now. External calendar changes still require approval.</p>
          <div className="mt-5 space-y-2">{replanPreview.slice(0,5).map((task,index)=><div key={task.id} className="flex items-center gap-3 rounded-2xl bg-white/45 px-4 py-3"><span className="grid h-7 w-7 place-items-center rounded-full bg-[#ecd9d2] text-xs">{index+1}</span><span className="min-w-0 flex-1 font-serif">{task.title}</span><span className="text-xs opacity-55">{priorityLabel(task.priority)}</span></div>)}</div>
          <div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={applyReplanPreview} className="rounded-full bg-[#3d302c] px-5 py-2.5 text-sm text-white">Use this order in Today</button><button type="button" onClick={()=>emitGlowOpen({prompt:'Help me replan the rest of today and show me the changes before committing them.',activeObject:'Replan My Day'})} className="rounded-full border border-[#d4bbb4] bg-white/50 px-5 py-2.5 text-sm">Plan it with Glow</button></div>
        </Modal>}
