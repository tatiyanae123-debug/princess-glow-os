...
export function UniversalCaptureDock(){
  ...
  return <div className="fixed bottom-4 right-4 z-[80] flex max-w-[calc(100vw-2rem)] flex-col items-end gap-2 sm:bottom-5 sm:right-5">
    ...
    <button type="button" onClick={()=>setOpen(current=>!current)} className="group inline-flex items-center gap-2 rounded-full border border-white/70 bg-[#d58f9a] px-4 py-3 text-[9px] font-semibold text-white shadow-[0_14px_38px_rgba(121,70,76,.25)] transition hover:-translate-y-0.5 hover:bg-[#bf7884]" aria-expanded={open} aria-label="Add anything to Glow OS"><Plus size={14}/><span>Add Anything</span><Sparkles size={11} className="opacity-80"/></button>
  </div>;
}
