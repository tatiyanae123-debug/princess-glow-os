...
function parseJson(text: string): AiIntakeAnalysis | null {
  ...
  try {
    ...
    if (!value || typeof value !== 'object' || typeof value.type !== 'string' || typeof value.title !== 'string') return null;
    return {
      type: value.type,
      title: value.title.slice(0, 160),
      confidence: typeof value.confidence === 'number' ? Math.max(0, Math.min(1, value.confidence)) : 0.8,
      destinations: Array.isArray(value.destinations) ? value.destinations.filter((x: unknown) => typeof x === 'string').slice(0, 8) : [],
