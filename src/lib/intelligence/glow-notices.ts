...
export async function ensureGlowNotices(userId:string){const snapshot=await buildCrossSystemSnapshot(userId,'observations');const existing=await db.select().from(glowNotices).where(and(eq(glowNotices.userId,userId),eq(glowNotices.status,'active')));const titles=new Set(existing.map(x=>x.title));const proposals:Array<{domain:string;title:string;evidence:string;recommendation:string;confidence:number;actionType:string}> = [];
...
const fresh=proposals.filter(p=>!titles.has(p.title));if(fresh.length)await db.insert(glowNotices).values(fresh.map(p=>({...p,userId,status:'active',actionPayload:{}})));return db.select().from(glowNotices).where(eq(glowNotices.userId,userId));}
