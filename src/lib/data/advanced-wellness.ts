...
export async function saveWellnessCheckIn(userId:string,input:{state:string;need:string;activation?:string|null;energy?:string|null;bodySignals?:string[];notes?:string}){
  const values={state:input.state.trim(),need:input.need.trim(),activation:input.activation?.trim()||null,energy:input.energy?.trim()||null,bodySignals:Array.from(new Set(input.bodySignals??[])).slice(0,12),notes:input.notes?.trim()||null};
  const [latest]=await db.select().from(wellnessCheckIns).where(eq(wellnessCheckIns.userId,userId)).orderBy(desc(wellnessCheckIns.createdAt)).limit(1);
  if(latest&&Date.now()-latest.createdAt.getTime()<10*60*1000){const [row]=await db.update(wellnessCheckIns).set(values).where(and(eq(wellnessCheckIns.id,latest.id),eq(wellnessCheckIns.userId,userId))).returning();return row??null;}
  const [row]=await db.insert(wellnessCheckIns).values({userId,...values}).returning();return row??null;
}
export async function startWellnessProtocol(userId:string,input:{protocolKey:string;title:string;mode:WellnessMode;queue:ProtocolStep[];beforeActivation?:string|null;context?:Record<string,unknown>}){
  const seen=new Set<string>();const queue=input.queue.filter(s=>s.id&&s.title&&Number.isFinite(s.seconds)&&!seen.has(s.id)&&seen.add(s.id)).map(s=>({...s,seconds:Math.max(0,Math.min(3600,Math.round(s.seconds)))})).slice(0,30);if(!queue.length)return null;
