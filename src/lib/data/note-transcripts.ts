...
export async function saveTranscriptChunk(userId:string,sourceId:string,input:{chunkIndex:number;text:string;startSeconds?:number|null;endSeconds?:number|null;analysis?:string|null}){
 const existing=(await db.select().from(noteTranscriptChunks).where(and(eq(noteTranscriptChunks.sourceId,sourceId),eq(noteTranscriptChunks.chunkIndex,input.chunkIndex))).limit(1))[0];
 if(existing){const [row]=await db.update(noteTranscriptChunks).set({text:input.text,startSeconds:input.startSeconds??existing.startSeconds,endSeconds:input.endSeconds??existing.endSeconds,analysis:input.analysis??existing.analysis}).where(eq(noteTranscriptChunks.id,existing.id)).returning();return row}
...
export async function saveAnalysis(userId:string,sourceId:string,input:{summary:string;keyPoints:string[];decisions:string[];actionItems:string[];questions:string[];themes:string[];model?:string}){
 ...
 if(existing){const [row]=await db.update(noteTranscriptAnalyses).set({...input,status:'complete',updatedAt:new Date()}).where(eq(noteTranscriptAnalyses.id,existing.id)).returning();return row}
...
export async function saveQuestion(userId:string,sourceId:string,input:{question:string;answer:string;evidence:Array<{chunkIndex:number;quote:string}>;model?:string}){const [row]=await db.insert(noteTranscriptQuestions).values({userId,sourceId,...input}).returning();return row}
