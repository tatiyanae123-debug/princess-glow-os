...
export type ConnectionsOverview = GoogleConnectionStatus & { calendarState: 'connected' | 'needs_reauthorization' | 'error' | 'disconnected'; lastSync: Awaited<ReturnType<typeof getLatestCalendarSync>> };
...
export async function getConnectionsOverview(userId: string): Promise<ConnectionsOverview> {
  ...
  const calendarState = !status.connected ? 'disconnected' : status.needsReauthorization || !status.hasCalendarScope || lastSync?.errorCode === 'revoked' ? 'needs_reauthorization' : lastSync?.status === 'error' ? 'error' : 'connected';
