...
export const GLOW_ENGINE_PRIORITIES:EnginePriority[]=engines.map((engine):EnginePriority=>{
 const families=familiesByEngine(engine).map(f=>f.id);
 const unlocks=families.length;
 const priority:EnginePriority['priority']=unlocks>=7?'P0':unlocks>=4?'P1':'P2';
 return {engine,unlocks,families,dependsOn:dependencies[engine]??[],priority};
}).sort((a,b)=>b.unlocks-a.unlocks||a.engine.localeCompare(b.engine));

export const GLOBAL_BUILD_ORDER=[
 'object-detail','library','timeline','intelligence','collection','builder','guided','progress','planner','studio','learn','review','object-projection','beauty-routine','beauty-maintenance',
] as const;
