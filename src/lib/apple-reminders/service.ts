...
export async function importAppleReminders(userId:string,payload:AppleReminderImport){
  ...
  for(const reminder of payload.reminders){
    await db.insert(appleReminders).values({userId,externalId:reminder.externalId,listName:reminder.listName,title:reminder.title,notes:reminder.notes??null,dueAt:reminder.dueAt?new Date(reminder.dueAt):null,completed:reminder.completed,lastSyncedAt:now,importAudit:{importedAt:now.toISOString(),source:'iphone_shortcuts'}}).onConflictDoUpdate({target:[appleReminders.userId,appleReminders.externalId],set:{listName:reminder.listName,title:reminder.title,notes:reminder.notes??null,dueAt:reminder.dueAt?new Date(reminder.dueAt):null,completed:reminder.completed,lastSyncedAt:now,importAudit:{importedAt:now.toISOString(),source:'iphone_shortcuts'}}});
    imported+=1;
  }
