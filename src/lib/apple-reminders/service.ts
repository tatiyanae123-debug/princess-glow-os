...
export async function importAppleReminders(userId:string,payload:AppleReminderImport){
  ...
  for(const reminder of payload.reminders){
    ...
    const sourceFields={
      ...
      personName:reminder.personName??null,
      recurrence:reminder.recurrence??null,
      notificationStyle:reminder.notificationStyle??null,
      spokenReminder:reminder.spokenReminder??null,
      smartTiming:reminder.smartTiming??null,
    ...
    const [saved]=await db.insert(appleReminders).values({userId,externalId:reminder.externalId,listName:reminder.listName,title:reminder.title,notes:reminder.notes??null,dueAt,completed:reminder.completed,lastSyncedAt:now,importAudit}).onConflictDoUpdate({target:[appleReminders.userId,appleReminders.externalId],set:{listName:reminder.listName,title:reminder.title,notes:reminder.notes??null,dueAt,completed:reminder.completed,lastSyncedAt:now,importAudit}}).returning();
