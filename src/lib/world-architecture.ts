...
const worlds: Record<WorldKey, Omit<WorldArchitecture, 'title' | 'eyebrow'>> = {
  ...
  beauty: { world:'beauty', statement:'A clean visual beauty system for routines, products, skincare, makeup, hair and progress.', insight:'Glow keeps today’s beauty routine in front while products, reactions and history stay one layer deeper.', actionLabel:'Open Beauty', actionHref:'/beauty', context:[{label:'Skincare',href:'/skincare'},{label:'Makeup',href:'/makeup'},{label:'Beauty Lab',href:'/beauty/lab'},{label:'Hair',href:'/hair'}] },
...
function routeTitle(pathname: string): { world: WorldKey; title: string; eyebrow: string } {
  ...
  if (pathname.startsWith('/skincare')) return {world:'beauty',title:'Skincare',eyebrow:'BEAUTY / SKINCARE'};
  if (pathname.startsWith('/makeup')) return {world:'beauty',title:'Makeup',eyebrow:'BEAUTY / MAKEUP'};
