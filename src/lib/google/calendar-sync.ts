...
export type CalendarSyncOutcome =
  | { ok: true; status: 'success' | 'partial'; count: number; cancelled: number }
...
export async function syncGoogleCalendar(userId: string): Promise<CalendarSyncOutcome> {
  ...
  if (!result.ok) {
    await db
      ...
      .set({ status: 'error', errorCode: result.reason, completedAt: new Date() })
      .where(and(eq(calendarSyncHistory.id, historyId), eq(calendarSyncHistory.userId, userId)));
    return result;
  }
  ...
  await db
    ...
    .set({
      status,
      calendarsRead: result.calendarsRead,
      eventsRead: result.events.length,
      eventsUpserted: result.events.length,
...
export async function getLatestCalendarSync(userId: string) {
  const [row] = await db
    ...
    .where(eq(calendarSyncHistory.userId, userId))
