...
function exactHref(type: string, id: string, extra?: string | null) {
  switch (type) {
    ...
    case 'Medication': return `/maintenance?medicationId=${encodeURIComponent(id)}`;
    case 'Supplement': return `/maintenance?supplementId=${encodeURIComponent(id)}`;
    case 'Link': return `/resources?linkId=${encodeURIComponent(id)}`;
...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  if (term) {
    ...
    results = [
      ...
      ...linkRows.map(x => ({ id: x.id, type: 'Link', title: x.title, subtitle: x.category, href: exactHref('Link', x.id) })),
    ];
  }
