...
function exactHref(type: string, id: string, extra?: string | null) {
  switch (type) {
    ...
    case 'Finance Goal': return `/finance/brain/goal/${encodeURIComponent(id)}`;
      ...
      <div className="mx-auto w-full max-w-[1180px] pb-12 pt-2">
        <section className="relative overflow-hidden rounded-[28px] border border-[#EEE7E4] bg-white px-6 py-9 shadow-[0_18px_60px_rgba(63,45,38,.055)] sm:px-9 lg:px-12 lg:py-12">
          ...
          <div className="relative max-w-[760px]">
            ...
            <p className="mt-4 max-w-[660px] text-[13px] leading-6 text-[#817771]">Search the same Glow OS you are already in. Every supported result opens the exact saved record instead of dropping you into a generic room.</p>
          </div>
          <form action="/search" className="relative mt-7 flex min-h-[54px] items-center gap-2 rounded-full border border-[#E9E1DE] bg-white/95 p-1.5 pl-4 shadow-[0_10px_32px_rgba(56,38,31,.06)] backdrop-blur-xl">
            ...
            <button type="submit" className="inline-flex h-10 items-center gap-1.5 rounded-full bg-[#C65F76] px-5 text-[12px] font-medium text-white">Search<ArrowRight size={13} /></button>
          </form>
        </section>

        {!term ? (
          <section className="mt-5 grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
            <div className="rounded-[22px] border border-[#EEE7E4] bg-white p-5">
              <p className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#8C837D]">Suggested questions</p>
              <div className="mt-3 divide-y divide-[#F1EBE8]">{SUGGESTED.map((item) => <Link key={item} href={`/search?q=${encodeURIComponent(item)}`} className="flex items-center justify-between gap-4 py-3 text-[12.5px] text-[#38322E]"><span>{item}</span><ArrowRight size={13} /></Link>)}</div>
            </div>
          ...
          <section className="mt-5 overflow-hidden rounded-[22px] border border-[#EEE7E4] bg-white">
            ...
            <div className="divide-y divide-[#F1EBE8]">
              {results.length ? results.map((result) => (
                <Link key={`${result.type}-${result.id}`} href={result.href} target={result.external ? '_blank' : undefined} rel={result.external ? 'noreferrer' : undefined} className="grid gap-2 px-5 py-4 transition hover:bg-[#F7EEED]/45 md:grid-cols-[120px_minmax(0,1fr)_110px]">
                  <span className="text-[9.5px] font-semibold uppercase tracking-[.12em] text-[#C36A7C]">{result.type}</span>
                  <div className="min-w-0"><p className="truncate text-[12.5px] font-medium text-[#342E2A]">{result.title}</p>{result.subtitle ? <p className="mt-1 line-clamp-2 text-[10.5px] leading-5 text-[#918781]">{result.subtitle}</p> : null}</div>
                  <span className="flex items-center justify-end gap-1 text-[10.5px] font-medium text-[#B95B70]">{result.external ? 'Open saved link' : 'Open exact record'} <ArrowRight size={12} /></span>
                </Link>
              )) : (
                <div className="px-5 py-12 text-center">
                  <p className="glow-display text-[23px] text-[#3B342F]">Nothing matched that search yet.</p>
                  <p className="mx-auto mt-2 max-w-[520px] text-[11px] leading-5 text-[#8B817B]">Try a different phrase, or ask Brain to interpret the question across your connected Glow rooms.</p>
                  <Link href={`/brain?q=${encodeURIComponent(term)}`} className="mt-5 inline-flex items-center gap-2 rounded-full bg-[#C45F76] px-4 py-2.5 text-[11px] font-medium text-white">Ask Brain <ArrowRight size={12} /></Link>
                </div>
              )}
            </div>
          </section>
