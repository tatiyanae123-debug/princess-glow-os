...
function clampBar(value:number){return Math.max(0,Math.min(100,value))}
...
export default async function SystemOverviewPage(){
  ...
  const completedTasks=tasks.filter(t=>t.status==='done');const recentSessions=sessions.filter(s=>s.occurredAt>=start);const activeHabits=habits.length;const habitPossible=activeHabits*7;const habitRate=habitPossible?Math.min(100,Math.round((habitLogs.length/habitPossible)*100)):0;
  ...
  const taskBar=completedTasks.length?Math.min(100,completedTasks.length*7):0;const workoutBar=recentSessions.length?Math.min(100,recentSessions.length*18):0;
  const activity=[['Completed tasks · all saved',completedTasks.length,taskBar],['Workouts · last 7 days',recentSessions.length,workoutBar],['Habit completion · last 7 days',activeHabits?`${habitRate}%`:'—',activeHabits?habitRate:0]] as const;
  ...
  const signalValues=[completedTasks.length*12,recentSessions.length*22,habitRate,readyConnections*40,tasks.length*3,sessions.length*4,habitLogs.length*2,(googleHealthy?55:0)+(appleHealthy?35:0)].map(clampBar);
  ...
  return <AppShell><div className="b9-system">
    ...
    <section className="b9-system-grid">
      <article className="b9-panel"><div className="flex items-center justify-between"><h2>Activity Overview</h2><span className="text-[8px] text-[#8e847d]">Each metric states its own time range</span></div><div className="b9-activity">{activity.map(([label,value,pct])=><div className="b9-activity-row" key={label}><span>{label}</span><div className="b9-track"><i style={{width:`${pct}%`}}/></div><strong>{value}</strong></div>)}</div><p className="b9-note mt-4">Glow does not invent device usage, backup status, cloud-storage percentages, or uptime metrics that the app cannot actually observe.</p></article>
      <article className="b9-panel"><div className="flex items-center gap-2"><Database size={12} className="text-[#7b3850]"/><h2>Data Footprint</h2></div><div className="b9-donut" style={{'--b9-pct':`${footprintPct}%`} as CSSProperties}><strong>{footprintTotal}</strong></div><p className="text-center text-[8px] text-[#8e847d]">records represented in this overview</p><div className="mt-3 grid gap-1 text-[8px]"><p className="flex justify-between"><span>Tasks</span><strong>{tasks.length}</strong></p><p className="flex justify-between"><span>Fitness sessions</span><strong>{sessions.length}</strong></p><p className="flex justify-between"><span>Habits</span><strong>{habits.length}</strong></p><p className="flex justify-between"><span>Habit logs · 7 days</span><strong>{habitLogs.length}</strong></p></div></article>
    </section>
    <section className="b9-system-foot">
      ...
      <article className="b9-panel"><div className="flex items-center gap-2"><Sparkles size={12} className="text-[#7b3850]"/><h2>Live Signals</h2></div>{signalValues.some(v=>v>0)?<div className="b9-spark">{signalValues.map((height,i)=><i key={i} style={{height:`${height}%`}}/>)}</div>:<div className="b9-empty-signal">No activity signal yet.</div>}<div className="mt-2 grid gap-1 text-[8px]"><p className="flex items-center gap-1"><ListChecks size={10}/> {completedTasks.length} completed tasks saved</p><p className="flex items-center gap-1"><Dumbbell size={10}/> {recentSessions.length} workouts in the last 7 days</p></div></article>
    </section>
  </div></AppShell>;
}
