...
export default async function InterviewModePage(){
  ...
  const now=Date.now();
  const interviewEvents=events.filter(e=>e.startAt.getTime()>=now&&`${e.title} ${e.description??''}`.toLowerCase().includes('interview')).sort((a,b)=>a.startAt.getTime()-b.startAt.getTime()).slice(0,4);
