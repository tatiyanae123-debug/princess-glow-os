...
function recency(date:Date|null|undefined){if(!date)return'';const d=Math.max(0,Math.floor((Date.now()-date.getTime())/86400000));return d<=0?'Today':d===1?'Yesterday':`${d} days ago`;}
...
export default async function BrainPage(){
 ...
 const[context,connections,domains,notes,memories]=await Promise.all([buildPersonalContext(userId).catch(()=>null),buildBrainConnections(userId),buildBrainMapDomains(userId),getNotesByUser(userId),getAllLifeMemoriesByUser(userId)]);
 ...
 const instances=connections.types.flatMap(t=>t.instances).sort((a,b)=>(b.occurredAt?.getTime()??0)-(a.occurredAt?.getTime()??0));
 ...
 const revelations=[...(context?.patterns??[]).slice(0,2).map(p=>({title:p.title,when:'Pattern'})),...(context?.recommendations??[]).slice(0,2).map(r=>({title:r.title,when:'Recommendation'})),...sortedNotes.slice(0,2).map(n=>({title:n.title||'Untitled note',when:recency(n.updatedAt)}))].slice(0,3);
 const active=instances.slice(0,4);const orbActive=connections.totalInstances>0||memoryCount>0||patternCount>0||insightCount>0;
 return <AppShell><div className="ir-world brain-world">
  ...
  <main className="brain-main">
   ...
   <section className="brain-insights ir-glass"><h3>✧ Today&apos;s Insights</h3>{todayInsights.length?todayInsights.map((v,i)=><p key={`${v}-${i}`}><span>{['◇','✦','✿'][i]??'✧'}</span>{v}</p>):<div className="ir-empty">No new insight signals yet.</div>}<Link href="/brain/insights">View All</Link></section>
