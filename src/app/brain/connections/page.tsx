...
export default async function BrainConnectionsPage(){
 ...
 return <AppShell><div className="b10-root b10-connection"><header className="b10-head"><div><p className="b10-eyebrow">4. BRAIN CONNECTIONS</p><h1>Brain Connection</h1><p>Trace real relationships across your Glow data.</p></div><Link href="/brain?addConnection=1" className="b10-primary"><Plus size={12}/> Add Connection</Link></header>
  <div className="b10-people-strip">{top.length?top.map(type=><Link href={`/brain/connections/${type.id}`} key={type.id}><span className="b10-person-avatar"><Link2 size={18}/></span><strong>{type.label}</strong><small>{BASIS_LABEL[type.basis]||type.basis}</small></Link>):<div className="b10-person-empty"><Link2 size={20}/><strong>No active connections yet</strong><small>Create links in Brain. Glow will not invent people, children, or relationships.</small></div>}<Link href="/brain?addConnection=1" className="b10-person-add"><Plus size={20}/><span>Add</span></Link></div>
  <div className="b10-tabs"><span className="active">Overview</span><Link href="/brain">Brain</Link><Link href="/calendar">Calendar</Link><Link href="/notes">Notes</Link><Link href="/wellness">Wellness</Link></div>
  <h2 className="b10-section-title">At a Glance</h2><div className="b10-connection-metrics">{[['Connection Types',connections.activeTypeCount],['Active Links',connections.totalInstances],['Stored Types',connections.types.filter(x=>x.basis==='stored').length],['Inferred Types',connections.types.filter(x=>x.basis==='inferred').length]].map(([label,value])=><div className="b10-card" key={String(label)}><strong>{value}</strong><span>{label}</span></div>)}</div>
