...
async function contextFor(userId: string, sourceRoute: string) {
  ...
  const openTasks = tasks.filter((task) => task.status !== 'done' && task.status !== 'cancelled').slice(0, 8).map((task) => ({ title: task.title, priority: task.priority, due: task.dueDate?.toISOString() ?? null }));
  const now = new Date();
  const upcomingEvents = events.filter((event) => event.startAt >= now).sort((a, b) => a.startAt.getTime() - b.startAt.getTime()).slice(0, 8).map((event) => ({ title: event.title, start: event.startAt.toISOString(), allDay: event.allDay, location: event.location }));
...
async function conversationalReply(input: { text: string; sourceRoute: string; selectedContext: string; history: HistoryTurn[]; userId: string }) {
  ...
  if (!apiKey) {
    ...
    return nextTask ? `You are in ${input.sourceRoute}. Your clearest open task is “${nextTask}”.${nextEvent ? ` Your next calendar item is “${nextEvent}”.` : ''}` : `You are in ${input.sourceRoute}. I do not see an open task to recommend right now.`;
  }
  const history = input.history.slice(-6).map((turn) => `${turn.role === 'user' ? 'Tatiyana' : 'Glow'}: ${String(turn.text ?? '').slice(0, 600)}`).join('\n');
  const instructions = `You are Glow, the persistent intelligent presence inside Glow OS. Be concise, practical, warm, and context-aware. You are not a separate destination: assist the room the user is already in. Never claim you changed data unless the system explicitly reports that an approved action completed. For requests that would change tasks, calendar, files, reminders, money, or external communication, say what you propose and wait for approval. Current room: ${input.sourceRoute}. Selected object/context: ${input.selectedContext || 'none'}. Current Glow context JSON: ${JSON.stringify(context)}. Recent conversation:\n${history || 'none'}\nUser: ${input.text}`;
  const response = await fetch('https://api.openai.com/v1/responses', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: process.env.OPENAI_GLOW_MODEL || 'gpt-5', input: [{ role: 'user', content: [{ type: 'input_text', text: instructions }] }], max_output_tokens: 500 }) });
...
export async function POST(request: Request) {
  try {
    ...
    if (!approved) return NextResponse.json({ ok: true, mode: 'proposal', requiresConfirmation: true, risk, actions: proposals, message: `Glow understood ${proposals.length} proposed change${proposals.length === 1 ? '' : 's'}. Review them before Glow acts.` });
    ...
    revalidatePath('/today'); revalidatePath('/tasks'); revalidatePath('/notes'); revalidatePath('/inbox');
    const summary = [...completed, ...queued.map((item) => `Queued for review: ${item}`)].join('. ');
    return NextResponse.json({ ok: true, mode: 'completed', requiresConfirmation: false, actions: proposals, message: summary || 'The approved request is complete.', receipt: { id: randomUUID(), summary: summary || 'Approved Glow action completed', destinations: [...new Set(proposals.flatMap((proposal) => proposal.destinations))] } });
  } catch (error) {
