...
function compactContext(context:PersonalContext){
 return {
  ...
  goals:context.activeGoals.slice(0,6).map(item=>item.title),
  recommendations:context.recommendations.slice(0,5).map(item=>({title:item.title,reason:item.reason,priority:item.priority,href:item.href})),
  ...
  patterns:context.patterns.slice(0,5).map(item=>({title:item.title,detail:item.detail,href:item.href})),
 };
}

function actionsFor(text:string,context:PersonalContext):QuickAction[]{
 const q=text.toLowerCase();const actions:QuickAction[]=[];
 const add=(label:string,href:string)=>{if(!actions.some(item=>item.href===href))actions.push({label,href})};
 ...
 if(/food|meal|grocery|eat/.test(q))add('Open Food','/food');
 if(/beauty|skin|makeup/.test(q))add('Open Beauty','/beauty');
 ...
 if(actions.length<2)add('See Today','/today');
...
function fallbackReply(text:string,context:PersonalContext){
 ...
 if(/what should i do|what do i do|right now|next thing|next hour/.test(q)){
  if(next)return `Right now, I’d do “${next.title}.” ${next.reason} ${event?`Your next calendar item is “${event.title}” at ${event.startAt.toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})}, so I’m protecting the time around it.`:'You do not have an upcoming calendar conflict showing right now.'}`;
  return `Your system looks fairly clear right now. I’d use the next block for one small task that reduces friction later, then stop instead of filling the time just because it is open.`;
 }
...
function extractOutputText(payload:unknown){
 ...
 for(const item of p?.output??[])for(const content of item.content??[])if(content.type==='output_text'&&content.text?.trim())return content.text.trim();
 return '';
}

async function modelReply(messages:ChatMessage[],context:PersonalContext){
 const key=process.env.OPENAI_API_KEY;if(!key)return null;
 const life=compactContext(context);const transcript=messages.slice(-12).map(message=>`${message.role==='user'?'User':'Glow'}: ${message.content}`).join('\n');
 const instructions=`You are Glow, the conversational intelligence layer inside a personal life operating system. Speak naturally, warmly, and directly. Sound like a highly capable human assistant, not a dashboard, robot, therapist, or motivational coach. Keep most answers to 2-5 short paragraphs unless the user asks for detail. Use the supplied live life context as the source of truth. Connect Calendar, Tasks, Reminders, Routines, Habits, Goals, and other signals instead of treating them separately. The core question is: what matters now, what realistically fits, and what can be ignored? Explain why when useful. Label predictions as estimates. Never say you completed, moved, deleted, sent, or scheduled something unless the app actually confirms that action. Never invent personal facts outside the supplied context. If the user is overwhelmed, simplify choices rather than adding more. Current page may be included in the user request.`;
 const input=`LIVE GLOW CONTEXT\n${JSON.stringify(life)}\n\nRECENT CONVERSATION\n${transcript}\n\nRespond to the user's latest message as Glow.`;
 try{
  const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Authorization':`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.GLOW_AI_MODEL||'gpt-5.6-luna',instructions,input,max_output_tokens:700})});
  if(!response.ok)return null;const payload=await response.json();return extractOutputText(payload)||null;
 }catch{return null}
}

export async function POST(request:Request){
 const session=await auth();if(!session?.user?.id)return NextResponse.json({error:'Unauthorized'},{status:401});
 let body:{messages?:ChatMessage[];page?:string};try{body=await request.json()}catch{return NextResponse.json({error:'Invalid request'},{status:400})}
 const messages=(body.messages??[]).filter(item=>(item.role==='user'||item.role==='assistant')&&typeof item.content==='string'&&item.content.trim()).slice(-16);
 const latest=[...messages].reverse().find(item=>item.role==='user')?.content.trim();if(!latest)return NextResponse.json({error:'Message required'},{status:400});
 ...
 const reply=await modelReply(augmented,context)??fallbackReply(latest,context);
 return NextResponse.json({reply,actions:actionsFor(latest,context),meta:{focusScore:context.focusScore,dayPart:context.dayPart,generatedAt:context.generatedAt.toISOString(),model:process.env.OPENAI_API_KEY?(process.env.GLOW_AI_MODEL||'gpt-5.6-luna'):'glow-local-intelligence'}});
}
