...
async function clearStaleAuthChecks() {
  ...
  for (const name of staleAuthCookies) store.delete(name);
}
...
export default async function SignInPage({ searchParams }: SignInPageProps) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.92),_transparent_36%),radial-gradient(circle_at_bottom_right,_rgba(218,210,255,0.28),_transparent_34%),linear-gradient(135deg,_#fbf7f1_0%,_#fffdf9_48%,_#f6f1f7_100%)] p-4">
      <div className="w-full max-w-sm rounded-[32px] border border-white/90 bg-white/55 p-8 shadow-[0_28px_90px_rgba(82,70,62,0.12),inset_0_1px_0_rgba(255,255,255,0.95)] backdrop-blur-2xl">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-neutral-500">
            {connectingContacts
              ? 'Allow read-only Contacts access once. Glow will return you directly to People.'
        ...
        <form
          action={async () => {
            'use server';
            await clearStaleAuthChecks();
            if (connectingContacts) {
              await signIn(
                'google',
        ...
        >
          ...
          >
            ...
            {connectingContacts ? 'Allow Contacts and return to People' : 'Continue with Google'}
          </button>
        </form>
