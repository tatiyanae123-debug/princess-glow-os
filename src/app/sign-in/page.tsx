...
async function clearStaleAuthChecks() {
  'use server';
  const store = await cookies();
  const names = [
    'authjs.state', '__Secure-authjs.state',
      ...
      <div className="w-full max-w-sm rounded-[30px] border border-slate-200/70 bg-white/80 p-8 shadow-[0_25px_80px_rgba(15,23,42,0.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-rose-400/20 text-rose-400">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3c-4.2 0-8 3.22-8 8.207 0 4.994 6 11.793 8 11.793s8-6.799 8-11.793C20 6.22 16.2 3 12 3z" /></svg>
          </div>
        ...
        <div className="space-y-3">
          {githubEnabled ? <form action={async () => { 'use server'; await clearStaleAuthChecks(); await signIn('github', { redirectTo: '/dashboard' }); }}>
            <button type="submit" className="flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-slate-900 px-4 py-3 text-sm font-medium text-white transition hover:bg-slate-800 dark:border-slate-700">
          ...
          {googleEnabled ? <form action={async () => { 'use server'; await clearStaleAuthChecks(); await signIn('google', { redirectTo: '/dashboard' }); }}>
            <button type="submit" className="flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-800 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700">
        ...
        <p className="mt-6 text-center text-xs text-slate-400 dark:text-slate-500">Your personal data stays private and scoped to your account.</p>
      </div>
    </div>
  );
}
