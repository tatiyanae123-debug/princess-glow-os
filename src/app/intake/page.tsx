...
export default async function IntakePage(){
 ...
 return <AppShell><div className="b8-intake-page space-y-3">
  <header><p className="b8-eyebrow">2. UNIVERSAL INTAKE</p><h1>Add Anything</h1><p className="mt-1 text-[10px] text-[#8A8078]">One place. All types. Instantly organized.</p></header>
  <section className="rounded-[8px] border border-[#eee5e1] bg-white p-3">
   <a href="#intake-workspace" className="block rounded-[7px] border border-dashed border-[#ddd0ca] bg-[#fffdfc] px-4 py-5"><p className="font-serif text-[14px] text-[#4b423d]">Add anything to Glow OS…</p><p className="mt-1 text-[8px] text-[#9A9088]">Attach, speak, or type.</p></a>
   <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-6">{[[UploadCloud,'Upload'],[Mic,'Voice'],[NotebookPen,'Note'],[Link2,'Link'],[FileImage,'Photo'],[ScanLine,'Scan']].map(([Icon,label])=>{const I=Icon as typeof UploadCloud;return <a href="#intake-workspace" key={String(label)} className="flex min-h-[40px] flex-col items-center justify-center gap-1 rounded-[6px] border border-[#eee5e1] bg-[#fbf8f6] text-[7.5px] text-[#675e59]"><I size={12}/>{String(label)}</a>})}</div>
  </section>
  ...
  <div className="flex items-center justify-between rounded-[8px] border border-[#eee5e1] bg-[linear-gradient(90deg,#fff,#f7ede8)] px-3 py-2"><div><p className="font-serif text-[10px]">Glow organizes it for you.</p><p className="text-[8px] text-[#8A8078]">You capture. Glow handles the rest.</p></div><Sparkles size={12} className="text-[#8e495a]"/></div>
 </div></AppShell>
}
