...
export async function buildMyDayAction(mode: 'standard' | 'lighter' = 'standard') {
  ...
  const commitments = calendarEvents.filter((event) => event.startAt >= start && event.startAt <= end).map((event) => ({ id: event.id, title: event.title, startAt: event.startAt, endAt: event.endAt }));
