...
export async function routeInboxItemAction(itemId:string):Promise<void>{const userId=await requireUserId();await routeInboxItem(userId,itemId);for(const path of ['/inbox','/today','/tasks','/notes','/goals','/calendar','/finance','/projects'])revalidatePath(path);}
