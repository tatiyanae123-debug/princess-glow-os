...
function summaryFor(kind:BriefingKind,context:Awaited<ReturnType<typeof buildPersonalContext>>){
  ...
  if(kind==='weekly')return `Weekly pulse: ${openTasks} open task${openTasks===1?'':'s'}, ${overdue} overdue, ${openReminders.length} open reminder${openReminders.length===1?'':'s'}, ${context.activeGoals.length} active goal${context.activeGoals.length===1?'':'s'}, and a current focus score of ${context.focusScore}.`;
