...
const groups=[
  ...
  {title:'Mind',paths:['/brain','/concierge','/memory','/observations','/graph','/notes','/knowledge']},
  {title:'Wellness',paths:['/wellness','/fitness','/food','/maintenance','/sleep','/symptoms']},
  {title:'Beauty',paths:['/beauty','/beauty/lab','/hair','/closet']},
  ...
  {title:'Home + World',paths:['/home','/all-rooms','/world','/travel']},
  {title:'Tools + System',paths:['/focus','/workout-studio','/intake','/gmail','/import','/connections','/notices','/settings']},
];
const byHref=new Map(navItems.map(item=>[item.href,item]));

export default function AllRoomsPage(){
  return <AppShell><div className="space-y-5">
    <header className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="glow-eyebrow text-[#C9727E]">Home + World</p><h1 className="glow-display mt-1 text-[42px] leading-[1.02] tracking-[-.025em] sm:text-[54px] lg:text-[60px]">All Rooms</h1><p className="mt-2 max-w-2xl text-[13px] leading-6 text-[#8A8078]">Every part of your life, connected. Use this as the complete directory while Life World gives you the immersive spatial view.</p></div><Link href="/world" className="inline-flex h-10 items-center justify-center rounded-full border border-[#EEE2DE] bg-white px-4 text-[11px] font-medium text-[#B85C70]">Enter Life World →</Link></header>
