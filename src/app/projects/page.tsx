...
import { ProjectExecutionStudio } from '@/components/projects/project-execution-studio';
...
export default async function ProjectsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }){
  ...
  return <AppShell><div className="space-y-4"><ProjectExecutionStudio projects={projects} tasks={tasks} goals={goals} events={events} memories={memories} planning={planning} nowIso={now.toISOString()}/>
    <details id="project-management" open={Boolean(requestedProjectId)} className="rounded-[26px] border border-white/70 bg-white/60 p-5 shadow-[0_18px_60px_rgba(83,73,82,.08)] backdrop-blur-xl"><summary className="cursor-pointer glow-display text-[18px]">Project management</summary>
