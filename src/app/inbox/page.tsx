  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-5xl space-y-5">
          ...
          <div className="divide-y divide-[#F1E7E3]">
            {open.length ? open.map((item) => {
                ...
                <article key={item.id} className="p-5">
                  <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-start">
                    <div>
                      ...
                      {destinations.length ? (
                        <div className="mt-3 flex flex-wrap gap-1.5">{destinations.map((destination) => <span key={destination} className="rounded-full border border-[#F1E7E3] bg-[#FDF8F6] px-2.5 py-1 text-[10.5px] text-[#8A8078]">Also relevant: {destination}</span>)}</div>
                      ) : null}
                    </div>
                    <form action={dismissInboxItemAction.bind(null, item.id)}>
                      <button type="submit" aria-label="Dismiss" title="Dismiss without routing" className="rounded-full border border-[#F1E7E3] bg-white p-2 text-[#8A8078] hover:bg-[#FDF8F6]"><X size={14} /></button>
                    </form>
                  </div>
                  <div className="mt-4">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
                      {options.map((option) => (
                        <form key={option.destination} action={routeInboxItemAction.bind(null, item.id)}>
                          <button name="destination" value={option.destination} type="submit" className={`flex w-full items-center justify-between gap-2 rounded-[12px] border px-3 py-2.5 text-[11px] font-medium transition ${option.recommended ? 'border-[#C9727E] bg-[#FBE4E8] text-[#B15A68] hover:bg-[#F5D3D8]' : 'border-[#F1E7E3] bg-white text-[#4A4440] hover:bg-[#FDF8F6]'}`}>
                            <span>{option.label}{option.recommended ? <span className="ml-1 text-[9px] uppercase text-[#B15A68]">Best</span> : null}</span><ArrowRight size={11} />
                          </button>
                        </form>
                      ))}
                    </div>
              ...
              <div className="p-10 text-center">
                <CheckCircle2 size={30} className="mx-auto text-[#5A6E52]" />
                <p className="mt-3 text-[13px] text-[#2B2420]">Inbox zero.</p>
                <p className="mt-1 text-[11.5px] text-[#8A8078]">Everything has been reviewed or dismissed.</p>
                <Link href="/intake" className="mt-4 inline-flex rounded-full border border-[#F1E7E3] bg-white px-4 py-2 text-[12px] text-[#8A8078] hover:bg-[#FDF8F6]">Add something new</Link>
              </div>
            )}
          </div>
        </section>
        {processed.length ? (
          <section className="rounded-[20px] border border-[#F1E7E3] bg-white p-5">
            <p className="text-[10.5px] font-semibold uppercase tracking-[.12em] text-[#8A8078]">Recent Decisions</p>
            <div className="mt-3 space-y-2">
              {processed.map((item) => (
                <div key={item.id} className="flex items-center gap-3 rounded-[12px] bg-[#FDF8F6] px-3 py-2 text-[12px] text-[#4A4440]">
                  <CheckCircle2 size={13} className="text-[#5A6E52]" /><span className="truncate">{item.suggestedTitle || item.rawText}</span><span className="ml-auto text-[10px] uppercase text-[#B5ACA5]">{item.routedEntityType || 'dismissed'}</span>
                </div>
              ))}
            </div>
          </section>
