...
import { addInboxItemFormAction, markInboxProcessedAction } from '@/app/actions/adaptive-os';
    ...
    <AppShell>
      <div className="mx-auto max-w-5xl space-y-5">
        <header>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-stone-500">Dump thoughts once. Glow classifies them so you do not have to decide which page they belong on first.</p>
        </header>
        ...
        <section className="rounded-[24px] border border-stone-200/70 bg-white/75 shadow-sm">
          <div className="flex items-center justify-between border-b border-stone-200/70 px-5 py-4"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-stone-600">Needs Processing</p><span className="text-xs text-stone-400">{open.length}</span></div>
          <div className="divide-y divide-stone-100">
            {open.length ? open.map((item) => (
              <div key={item.id} className="grid gap-4 p-5 md:grid-cols-[1fr_180px_120px] md:items-center">
                ...
                <form action={markInboxProcessedAction.bind(null, item.id)}><button type="submit" className="w-full rounded-xl border border-stone-200 px-3 py-2 text-xs text-stone-700 hover:bg-stone-50">Mark Routed</button></form>
              </div>
        ...
        {processed.length ? <section className="rounded-[24px] border border-stone-200/70 bg-white/60 p-5"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-stone-500">Recently Processed</p><div className="mt-3 space-y-2">{processed.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-xl bg-stone-50 px-3 py-2 text-xs text-stone-600"><CheckCircle2 size={13} className="text-emerald-600" /><span className="truncate">{item.suggestedTitle || item.rawText}</span></div>)}</div></section> : null}
      </div>
    </AppShell>
  );
}
