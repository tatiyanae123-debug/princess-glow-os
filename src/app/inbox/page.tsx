...
export default async function InboxPage() {
  ...
  return <AppShell><div className="mx-auto max-w-5xl space-y-5">
    <header><div className="flex items-center gap-2 text-rose-700"><Inbox size={18}/><p className="text-[10px] font-bold uppercase tracking-[.2em]">Universal Capture</p></div><h1 className="mt-2 text-4xl tracking-[-.04em] text-stone-950" style={{fontFamily:'var(--glow-font-display)'}}>Glow Inbox</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-stone-500">Dump thoughts once. Glow classifies them, then lets you explicitly route each item into the correct systems.</p></header>
    ...
    <section className="rounded-[24px] border border-stone-200/70 bg-white/75 shadow-sm"><div className="flex items-center justify-between border-b border-stone-200/70 px-5 py-4"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-stone-600">Needs Routing</p><span className="text-xs text-stone-400">{open.length}</span></div><div className="divide-y divide-stone-100">{open.length?open.map(item=><div key={item.id} className="grid gap-4 p-5 md:grid-cols-[1fr_180px_210px] md:items-center"><div><p className="text-sm font-medium text-stone-900">{item.suggestedTitle||item.rawText}</p><p className="mt-1 text-[10px] text-stone-500">Source: {item.source} · Confidence {Math.round(item.confidence*100)}%</p></div><div className="rounded-full bg-rose-50 px-3 py-2 text-center text-[10px] font-bold uppercase tracking-[.12em] text-rose-800">{item.suggestedType||'note'}</div><div className="flex gap-2"><form action={routeInboxItemAction.bind(null,item.id)} className="flex-1"><button type="submit" className="flex w-full items-center justify-center gap-2 rounded-xl bg-stone-900 px-3 py-2 text-xs text-white hover:bg-rose-950">Route it <ArrowRight size={12}/></button></form><form action={dismissInboxItemAction.bind(null,item.id)}><button type="submit" aria-label="Dismiss" className="rounded-xl border border-stone-200 bg-white p-2 text-stone-500 hover:bg-stone-50"><X size={14}/></button></form></div></div>):<div className="p-10 text-center"><CheckCircle2 size={30} className="mx-auto text-emerald-600"/><p className="mt-3 text-sm text-stone-700">Inbox zero.</p><p className="mt-1 text-xs text-stone-400">Everything has been processed.</p></div>}</div></section>
