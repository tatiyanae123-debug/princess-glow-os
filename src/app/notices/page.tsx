...
import { dismissGlowNoticeAction, reviewGlowNoticeAction, setGlowNoticeFeedbackAction, snoozeGlowNoticeAction } from '@/app/actions/glow-notices';
...
export default async function NoticesPage(){
  ...
  const highConfidence=active.filter(n=>n.confidence>=.8).slice(0,4);
  ...
  return <AppShell><div className="space-y-5">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      <Card><div className="flex items-center justify-between"><div className="flex items-center gap-2"><BellRing size={15} className="text-[#C9727E]"/><h2 className="glow-display text-[18px]">Active Notices</h2></div><span className="text-[10px] text-[#C9727E]">{active.length}</span></div><div className="mt-4 divide-y divide-[#F1E7E3]">{active.length?active.map(n=><article key={n.id} className="py-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#C9727E]"/><p className="text-[11.5px] font-medium text-[#3A332E]">{n.title}</p></div><p className="mt-1 pl-4 text-[10px] text-[#9A9088]">{n.domain} · {Math.round(n.confidence*100)}% confidence</p><p className="mt-2 line-clamp-2 pl-4 text-[10.5px] leading-4 text-[#7D746F]">{n.recommendation||n.evidence}</p></div><div className="flex gap-1"><form action={reviewGlowNoticeAction.bind(null,n.id)}><button title="Review suggested area" aria-label={`Review ${n.title}`} className="rounded-full border border-[#F1E7E3] p-2 text-[#5A6E52]"><WandSparkles size={11}/></button></form><form action={snoozeGlowNoticeAction.bind(null,n.id)}><button title="Snooze for 24 hours" aria-label={`Snooze ${n.title}`} className="rounded-full border border-[#F1E7E3] p-2 text-[#9A7A3D]"><Clock3 size={11}/></button></form><form action={dismissGlowNoticeAction.bind(null,n.id)}><button title="Dismiss" aria-label={`Dismiss ${n.title}`} className="rounded-full border border-[#F1E7E3] p-2 text-[#B15A68]"><X size={11}/></button></form></div></div></article>):<p className="py-8 text-center text-[11.5px] text-[#9A9088]">Nothing needs your attention.</p>}</div></Card>
      <Card><div className="flex items-center gap-2"><Sparkles size={14} className="text-[#C9727E]"/><h2 className="glow-display text-[18px]">High-Confidence Signals</h2></div><p className="mt-2 text-[10px] leading-4 text-[#A79D96]">This is derived from confidence, not a user pin state.</p><div className="mt-4 space-y-4">{highConfidence.length?highConfidence.map(n=><div key={n.id} className="rounded-[12px] bg-[#FFF8F5] p-3"><p className="text-[11.5px] font-medium">{n.title}</p><p className="mt-1 text-[9.5px] text-[#9A9088]">{n.domain} · {Math.round(n.confidence*100)}%</p></div>):<p className="text-[11px] text-[#9A9088]">No active signal is above the high-confidence threshold.</p>}</div></Card>
    </section>
    ...
    <section className="grid gap-4 lg:grid-cols-[1.2fr_.8fr_.65fr]">
      ...
      <Card><h2 className="glow-display text-[18px]">Notice Behavior</h2><div className="mt-4 space-y-4 text-[11px]"><p className="flex justify-between gap-3"><span>Confidence</span><span className="text-[#5A6E52]">Shown on each notice</span></p><p className="flex justify-between gap-3"><span>Snooze</span><span className="text-[#5A6E52]">24 hours</span></p><p className="flex justify-between gap-3"><span>Feedback</span><span className="text-[#5A6E52]">Saved on decisions</span></p></div><Link href="/settings/intelligence" className="mt-5 inline-flex text-[10.5px] text-[#C9727E]">Open intelligence controls →</Link></Card>
