...
export default async function ConnectionsPage(){
 ...
 const growth:number[]=[];const growthSeen=new Set<string>();[...raw].reverse().forEach(m=>{growthSeen.add(m.from.toLowerCase());growth.push(growthSeen.size)});const maxGrowth=Math.max(1,...growth);
 ...
 return <AppShell><div className="b9-connections-page">
  ...
  <nav className="b9-c-tabs" aria-label="Connection views"><span className="active">People</span><span>Mentors</span><span>Teams</span><span>Groups</span><span>Collabs</span></nav>
  <section className="b9-c-metrics"><article><small>Recent People</small><strong>{people.length}</strong></article><article><small>Ready Sources</small><strong>{connectedCount}</strong></article><article><small>Follow Ups</small><strong>{attention}</strong></article><article><small>Inbox Sample</small><strong>{raw.length}</strong></article></section>
