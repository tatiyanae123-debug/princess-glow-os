...
export default async function DashboardPage() {
  ...
  if (!process.env.DATABASE_URL) {
    return <AppShell><LivingDashboard data={getFallbackData()} error="DATABASE_URL is not configured." userName={userName} userImage={userImage} /></AppShell>;
  }

  try {
    const { getLivingDashboardData } = await import('@/lib/dashboard/living-dashboard');
    const data = await getLivingDashboardData(userId);
    return <AppShell><LivingDashboard data={data} userName={userName} userImage={userImage} /></AppShell>;
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return <AppShell><LivingDashboard data={getFallbackData()} error={message} userName={userName} userImage={userImage} /></AppShell>;
  }
}
