...
import { FoodIntelligenceStudioV3 } from '@/components/food/food-intelligence-studio-v3';
...
export default async function FoodPage() {
  ...
  const [state, finance, events] = await Promise.all([
    getFoodIntelligenceState(userId),
  ...
  return (
    <AppShell>
      <FoodIntelligenceStudioV3
        initial={serialized}
        nextEvent={nextEvent ? { id: nextEvent.id, title: nextEvent.title, startAt: nextEvent.startAt.toISOString() } : null}
        foodSpendCents={foodSpendCents}
        foodPurchaseCount={monthEntries.length}
      />
    </AppShell>
  );
}
