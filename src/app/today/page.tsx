...
import { TodayLivingCenterV3 } from '@/components/today-living-center-v3';
...
export default async function TodayPage(){
  ...
  const todaysEvents=events.filter(e=>e.startAt.toDateString()===now.toDateString()).sort((a,b)=>{if(a.allDay&&!b.allDay)return-1;if(!a.allDay&&b.allDay)return 1;return a.startAt.getTime()-b.startAt.getTime();});
  const latestWellness=wellnessEntries[0]??null;
  const routines=beautyRoutines.filter(r=>r.timeOfDay==='morning'||r.timeOfDay==='afternoon'||r.timeOfDay==='evening'||r.timeOfDay==='night').slice(0,8);

  return <TodayLivingCenterV3
    tasks={ranked.slice(0,8).map(t=>({id:t.id,title:t.title,priority:t.priority,dueLabel:taskDueLabel(t.dueDate,now)}))}
    events={todaysEvents.slice(0,8).map(e=>({id:e.id,title:e.title,timeLabel:e.allDay?'All day':e.startAt.toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'}),location:e.location}))}
    routines={routines.map(r=>({id:r.id,name:r.name,timeOfDay:r.timeOfDay}))}
    energy={levelToNumber(latestWellness?.energy)} mood={levelToNumber(latestWellness?.mood)} sleepHours={latestWellness?.sleepHours??null}
  />;
}
