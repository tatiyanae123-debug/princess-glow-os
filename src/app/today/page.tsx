...
export default async function TodayPage() {
  ...
  return <AppShell><div className="mx-auto max-w-[1460px] space-y-5">
    <header className="rounded-[22px] border border-[#e5d8d0] bg-[linear-gradient(120deg,#f8ece8,#fffaf6_55%,#eee6d8)] p-6 sm:p-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[8px] font-bold uppercase tracking-[.22em] text-[#a66a74]">TODAY · ADAPTIVE ACTION LAYER</p><h1 className="glow-display mt-2 text-[36px] leading-none text-[#372c28] sm:text-[48px]">Only show me what matters now.</h1><p className="mt-3 max-w-2xl text-[11px] leading-5 text-[#78665f]">Glow combines tasks, calendar, wellness, routines, Life Modes, personal rules and focus history so you can act without deciding where to look first.</p></div><div className="flex flex-col gap-2"><div className="rounded-[16px] border border-white/70 bg-white/55 px-4 py-3"><p className="text-[7px] uppercase tracking-[.16em] text-[#9a837b]">Available block</p><p className="glow-display mt-1 text-[24px] text-[#3d312d]">{availableMinutes==null?'Open':`${availableMinutes} min`}</p><p className="mt-1 text-[8px] text-[#8c7770]">{nextEvent?`before ${nextEvent.title}`:'no upcoming constraint'}</p></div><div className="flex gap-2"><Link href="/tomorrow" className="rounded-[9px] border border-[#dfd0c7] bg-white/65 px-3 py-2 text-center text-[8px] text-[#705c55]">Prepare Tomorrow</Link><Link href="/focus" className="rounded-[9px] bg-[#3e332e] px-3 py-2 text-center text-[8px] text-white">Focus Mode</Link></div></div></div>
    </header>

    <AdaptiveTodayPanel userId={userId}/>

    <section className="rounded-[18px] border border-[#e5d8d0] bg-white/45 px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[7px] font-bold uppercase tracking-[.16em] text-[#9c7f77]">Detailed day view</p><p className="mt-1 text-[9px] text-[#74615a]">The adaptive layer above decides. The sections below show the evidence and supporting detail.</p></div><div className="flex gap-2"><Link href="/rules" className="rounded-full border border-[#dfd1c9] px-3 py-1.5 text-[8px] text-[#725f58]">Personal Rules</Link><Link href="/maintenance" className="rounded-full border border-[#dfd1c9] px-3 py-1.5 text-[8px] text-[#725f58]">Coming Soon</Link><Link href="/notices" className="rounded-full border border-[#dfd1c9] px-3 py-1.5 text-[8px] text-[#725f58]">Glow Notices</Link></div></div></section>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.5fr_.9fr]">
      ...
      <aside className="space-y-4">
        <section className="editorial-surface overflow-hidden"><div className="flex items-center gap-2 border-b border-[#eadfd6] px-4 py-3"><Sparkles size={13} className="text-[#aa7780]"/><p className="text-[8px] font-bold uppercase tracking-[.16em] text-[#745f58]">GLOW CONTEXT</p></div><div className="p-4"><p className="glow-display text-[18px] leading-6 text-[#433631]">{snapshot.message}</p><p className="mt-2 text-[8px] leading-4 text-[#8e7770]">This is generated from live cross-system context, not a decorative message.</p><Link href="/notices" className="mt-4 inline-flex items-center gap-1 text-[8px] text-[#a15f69]">See actionable notices <ArrowRight size={9}/></Link></div></section>
