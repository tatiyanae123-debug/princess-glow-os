...
export const glowEntities = pgTable('glow_entities', {
  ...
  availableActions: jsonb('available_actions').$type<string[]>().notNull().default([]),
  resources: jsonb('resources').$type<string[]>().notNull().default([]),
  crossDomainEffects: jsonb('cross_domain_effects').$type<string[]>().notNull().default([]),
  permissions: jsonb('permissions').$type<Record<string, unknown>>().notNull().default({}),
  completionStage: text('completion_stage'),
  ...
  userTypeIdx: index('glow_entities_user_type_idx').on(t.userId, t.entityType),
  sourceUnique: uniqueIndex('glow_entities_source_uidx').on(t.userId, t.sourceTable, t.sourceId),
  canonicalUnique: uniqueIndex('glow_entities_canonical_uidx').on(t.userId, t.canonicalKey),
}));
